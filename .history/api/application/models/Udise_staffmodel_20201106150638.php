...
function delete_nsqf_trainee_staff_dtl($id){
 ...
 // ('High School','Higher Secondary School')
  ...
  public function Teacher_training_det($dist,$b,$school_type,$training_mode)
  {
    if($training_mode==1)
    {
     $SQL="select sc.block_name, u.teacher_id,u.teacher_name, t.type_teacher, sc.udise_code, sc.school_name,sc.district_name,count(1) as total,tt.training,td.trained_at,sum(training_days) as Days from students_school_child_count sc
 ...
 where sc.school_type_id in ($school_type) and u.archive = 1 and sc.district_id = $dist and t.category = 1 group by $b, tt.training,td.trained_at having Days >0";
    }
    else if($training_mode==2)
    {
     $SQL="select sc.block_name, u.teacher_id, u.teacher_name, t.type_teacher, sc.udise_code, sc.school_name,sc.district_name,count(1) as total, sum(training_days) as Days from students_school_child_count sc
 left join udise_staffreg u on u.school_key_id = sc.school_id and sc.school_type_id in (1,2,4) and u.archive = 1
 join teacher_type t on t.id = u.teacher_type
 left join teacher_training_details td on td.teacher_code = u.teacher_code and u.archive = 1
 left join teacher_training tt on tt.id = td.training_type
 where sc.school_type_id in ($school_type) and u.archive = 1 and sc.district_id = $dist and t.category = 1 group by $b having Days is Null or Days = 0";
    }
