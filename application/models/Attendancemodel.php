...
class Attendancemodel extends CI_Model
{
	...
	/*public function overallist($school_id){
		$sql="select * from schoolnew_attend_summary where school_id=".$school_id.";";
		$query= $this->db->query($sql);
		return $query->result_array();
	}*/
	public function overallllist($school_id,$date){
		$sql="select 
ROUND(((sum(session1_maleP)/sum(session1_all))*100),2) AS malepre,
ROUND(((sum(session1_femaleP)/sum(session1_all))*100),2) AS femalepre,
ROUND(((sum(session1_maleA)/sum(session1_all))*100),2) AS maleab,
ROUND(((sum(session1_femaleA)/sum(session1_all))*100),2) AS femaleab
from 
...
//echo $sql; die();
    ...
    public function overallclasswise($school_id,$date){
		...
		femalepre,
		maleab,
		ROUND(100-malepre-femalepre-maleab,2) AS femaleab, 
		(malepre+femalepre) as clspre,
		(maleab+ROUND(100-malepre-femalepre-maleab,2)) as clsab
...
left join teachers_attend_daily on cast(teachers_attend_daily.teacher_code as unsigned)=udise_staffreg.u_id and teachers_attend_daily.teacher_code!='' and teachers_attend_daily.teacher_code!=null and teachers_attend_daily.school_id=".$school_id." 
...
case when incinerator=1 then 'Available' when incinerator=0 then 'Not Available' when incinerator='' then 'Not Available' when incinerator is null then 'Not Available' end as incinerator
 ...
 return $query->result_array();
	...
	public function schemeindent($tablename,$school_id){
		$sql="select scheme_name,sum(totalapplied) as totalapplied,sum(strength) as strength,concat(round(sum(totalapplied)/sum(strength)*100,2),'%') as overall from (select schoolnew_schemeindent.class,scheme_name,count(1) as totalapplied,strength from ".$tablename."
