...
class Beo_blockmodel extends CI_Model
{
...
function getalldistrictcountsbyclassblock($block_id){
 ...
 function getallstudentsbyschid($schoolid,$classid){
    ...
    ***************/
    ...
    public function get_block_name($block_id,$beo_id)
  {
...
where schoolnew_basicinfo.block_id=".$blckid." AND schoolnew_basicinfo.curr_stat=1 group by community_name order by community_code";
   ...
   function getStudentTotalCountByBlock($blckid){
    ...
    function getallrenewallist($where,$ts=' GROUP BY school_key_id'){
        $SQL = "SELECT 
    T1.*,
    ...
    (CASE schoolnew_renewapprove.renewal_id WHEN T1.id THEN
		CASE WHEN schoolnew_renewapprove.auth IS NULL THEN
			DATEDIFF(DATE(NOW()),T1.createddate)
		ELSE
			...
			ELSE 
				CASE schoolnew_renewapprove.auth WHEN 2 THEN
					DATEDIFF(DATE(NOW()),IFNULL((SELECT timestamp FROM schoolnew_renewapprove WHERE renewal_id=T1.id AND auth=2 ORDER BY timestamp DESC LIMIT 1),T1.createddate))
				ELSE
					CASE schoolnew_renewapprove.auth WHEN 3 THEN
						DATEDIFF(DATE(NOW()),IFNULL((SELECT timestamp FROM schoolnew_renewapprove WHERE renewal_id=T1.id AND auth=3 ORDER BY timestamp DESC LIMIT 1),T1.createddate))
					ELSE 
						CASE schoolnew_renewapprove.auth WHEN -1 THEN
                            DATEDIFF(DATE(NOW()),IFNULL((SELECT timestamp FROM schoolnew_renewapprove WHERE renewal_id=T1.id AND auth=2 ORDER BY timestamp DESC LIMIT 1),T1.createddate))
                        ELSE
		...
		END
    ELSE 
		CASE WHEN (SELECT renewal_id FROM schoolnew_renewapprove WHERE renewal_id=T1.id) IS NULL THEN
			DATEDIFF(DATE(NOW()),T1.createddate)
		ELSE
			NULL
		END
    ...
    FROM 
        (SELECT 
            ...
            (CASE schoolnew_renewcategory.applied_category WHEN 1 THEN 'RENEWAL' ELSE
                CASE schoolnew_renewcategory.applied_category WHEN 2 THEN 'UPGRADE' ELSE
                    CASE schoolnew_renewcategory.applied_category WHEN 3 THEN 'ADDITIONAL CLASSES' ELSE NULL END
                END
            END) AS appliedfor,
            ...
            (CASE schoolnew_renewal.condsatisfied WHEN 1 THEN 'FULLY SATISFIED' ELSE 
                CASE schoolnew_renewal.condsatisfied WHEN 2 THEN 'PARTLY SATISFIED' ELSE 
                    CASE schoolnew_renewal.condsatisfied WHEN 3 THEN 'NONE SATISFIED' ELSE 
                        CASE schoolnew_renewal.condsatisfied WHEN 4 THEN 'NOT APPLICABLE' ELSE NULL END 
                    END 
                END 
            END) AS conditionstatisfied, 
            (CASE schoolnew_academicinfo.low_class WHEN 15 THEN 'PRE-KG' ELSE 
                CASE schoolnew_academicinfo.low_class WHEN 14 THEN 'LKG' ELSE 
                    CASE schoolnew_academicinfo.low_class WHEN 13 THEN 'UKG' ELSE schoolnew_academicinfo.low_class END 
                END 
            END) AS low_class, 
            schoolnew_academicinfo.high_class, 
            ...
            JOIN schoolnew_academicinfo ON schoolnew_academicinfo.school_key_id=schoolnew_renewal.school_key_id 
            WHERE schoolnew_renewal.timestamp IN (SELECT MAX(timestamp) FROM schoolnew_renewal ".$ts.") 
            ...
            schoolnew_renewal ".$ts.") AND (id IN (SELECT renewal_id FROM schoolnew_renewapprove WHERE auth is NOT NULL))) GROUP BY fileid,approveid ORDER BY ts ASC";
    ...
    function checkauthendication($auth,$renewalID){
      ...
      public function get_block_school_student_teacher_total($block_id,$beo_id)
          {
print_r($block_name);
         ...
         * 26/01/2019
         **/
          public function get_block_school_type_based_schoolinfo($block_name,$beo_map)
          {
              ...
              $over_school_info['result'] = $result;
                ...
                {
                  $school_info[] = $this->get_block_school_type_count($res->school_type,$block_name,$beo_map);
                }
          ...
          public function get_block_school_type_count($school_type,$block_name,$beo_map)
          {
           ...
           ->from('teacher_profile_count')
             ->join('schoolnew_basicinfo','schoolnew_basicinfo.school_id=teacher_profile_count.school_key_id')
            ->join('students_school_child_count','students_school_child_count.school_id=teacher_profile_count.school_key_id','right')
             ->where('schoolnew_basicinfo.beo_map',$beo_map)
            ->where('block_name',$block_name)
   ...
   function get_teaching_block_school($block_id,$cate_type,$beo_id){
      
         $this->db->select("
       sum(case when school_type='Un-aided' then (teach_mle+teach_fmle) else 0 end) as unaided,
       ...
       sum(case when school_type='Central Govt' then (teach_mle+teach_fmle) else 0 end) as CentralGovt,students_school_child_count.school_name,students_school_child_count.block_id,students_school_child_count.udise_code,students_school_child_count.block_name,students_school_child_count.district_name");
        $this->db->from('teacher_profile_count');
       $this->db->join('schoolnew_basicinfo','schoolnew_basicinfo.school_id=teacher_profile_count.school_key_id','right');
     ...
     $this->db->where('schoolnew_basicinfo.beo_map',$beo_id);
  ...
  function get_nonteaching_block_school($block_id,$cate_type,$beo_id){
       ...
       sum(case when school_type='Central Govt' then (nonteach_mle+nonteach_fmle) else 0 end) as CentralGovt,students_school_child_count.school_name,students_school_child_count.block_id,students_school_child_count.udise_code,students_school_child_count.block_name,students_school_child_count.district_name");
       $this->db->from('teacher_profile_count');
       $this->db->join('schoolnew_basicinfo','schoolnew_basicinfo.school_id=teacher_profile_count.school_key_id','right');
      $this->db->join('students_school_child_count','students_school_child_count.school_id=teacher_profile_count.school_key_id','right');
     $this->db->where('students_school_child_count.block_id',$block_id);
...
if($result->low_class == 15)
  ...
  /*************************************** End ***********************************/
 function getalldistrictcountsbyteacherblock($block_id,$beo_id){
    $this->db->select('students_school_child_count.school_name,students_school_child_count.district_name,students_school_child_count.block_id,students_school_child_count.block_name,sum(teach_bt_mle+teach_bt_fmle) as BTteacher,sum(teach_sg_mle+teach_sg_fmle) as SGteacher,sum(teach_pg_mle+teach_pg_fmle) as PGteacher,sum(teach_hm_fmle+teach_hm_mle) as HM ,sum(teach_othr_mle+teach_othr_fmle) as Others,sum(teach_hm_fmle+teach_hm_mle+teach_bt_mle+teach_bt_fmle+teach_sg_mle+teach_sg_fmle+teach_pg_mle+teach_pg_fmle+teach_othr_mle+teach_othr_fmle) as Govteacher,schoolnew_basicinfo.block_id,schoolnew_basicinfo.beo_map')
    ->from('teacher_profile_count')  
    ->join('schoolnew_basicinfo','schoolnew_basicinfo.school_id=teacher_profile_count.school_key_id')
    ->join('students_school_child_count','students_school_child_count.school_id=teacher_profile_count.school_key_id','right')
         ->where('students_school_child_count.school_type','Government')
   ...
   function get_block_district_name($block_id)
     {
      $this->db->select('block_name,district_name')
      ->from('students_school_child_count')
      ...
      $query=$this->db->get();
       ...
       public function get_classwise_student_disability_school($block_name,$beo_id)
      {
...
JOIN schoolnew_basicinfo ON schoolnew_basicinfo.school_id=students_child_detail.school_id
where students_child_detail.transfer_flag=0 AND students_school_child_count.block_name='".$block_name."' AND schoolnew_basicinfo.beo_map=".$beo_id." group by school_name;"; 
//print_r($SQL);
//die();
       $query = $this->db->query($SQL);
       return $query->result();
      ...
      public function get_districtwise_report($user_type)
    {
      ...
      public function get_blockwise_RTE_student($block_id)
      {
...
//die();
       ...
       public function get_schoolwise_RTE_student($block_id)
      {
        ...
        $SQL=" select sc.school_name,sc.udise_code,sc.school_type,SUM(class_studying_id=15) as pre_kg,SUM(class_studying_id=13) as lkg,SUM(class_studying_id=14) as ukg,SUM(class_studying_id=1) as c1,SUM(class_studying_id=2) as c2, SUM(class_studying_id=3) as c3,SUM(class_studying_id=4) as c4, SUM(class_studying_id=5) as c5,SUM(class_studying_id=6) as c6,SUM(class_studying_id=7) as c7,SUM(class_studying_id=8) as c8, SUM(class_studying_id=9) as c9,SUM(class_studying_id=10) as c10, SUM(class_studying_id=11) as c11,SUM(class_studying_id=12) as c12 from students_child_detail st 
join students_school_child_count sc on sc.school_id=st.school_id where sc.block_name='".$block_id."' and st.transfer_flag=0 and st.child_admitted_under_reservation='yes' and sc.school_type_id in (2,3,4) group by sc.school_name;"; 
//print_r($SQL);
//die();
       $query = $this->db->query($SQL);
       return $query->result();
      ...
      public function get_state_renewal_details($block_id,$beo_id)
           {
            $SQL="SELECT 
  (SELECT 
    COUNT(*)
  FROM 
    schoolnew_renewal 
        JOIN schoolnew_basicinfo ON schoolnew_basicinfo.school_id=schoolnew_renewal.school_key_id
        join schoolnew_school_department ON schoolnew_school_department.id=schoolnew_basicinfo.sch_directorate_id 
        WHERE schoolnew_basicinfo.block_id=".$block_id."  AND schoolnew_school_department.id in (select sch_directorate_id from schoolnew_basicinfo where department like '%Directorate%of%Elementary%Education%') AND schoolnew_renewal.id NOT IN (SELECT renewal_id FROM schoolnew_renewapprove)
  ) AS BEO,
  (SELECT 
    COUNT(*) 
  FROM 
    ...
    JOIN schoolnew_block ON schoolnew_block.id=schoolnew_basicinfo.block_id
    join schoolnew_school_department ON schoolnew_school_department.id=schoolnew_basicinfo.sch_directorate_id 
        WHERE schoolnew_basicinfo.block_id=".$block_id."  AND (department like '%Directorate%of%School%Education%' or  department like '%Directorate%of%Matriculation %Schools%') AND schoolnew_renewal.id NOT IN (SELECT renewal_id FROM schoolnew_renewapprove WHERE (auth=3 OR auth=-3 OR auth IS NULL) )
  ...
  FROM 
    schoolnew_renewal 
        JOIN schoolnew_basicinfo ON schoolnew_basicinfo.school_id=schoolnew_renewal.school_key_id
        WHERE schoolnew_basicinfo.block_id=".$block_id."  AND schoolnew_renewal.id IN (SELECT renewal_id FROM schoolnew_renewapprove WHERE auth=1) AND schoolnew_renewal.vaild_upto IS NOT NULL
  ) AS APPROVED,
  ...
  FROM 
    ...
    WHERE schoolnew_basicinfo.block_id=".$block_id." AND schoolnew_basicinfo.beo_map=".$beo_id."  AND vaild_from IS NULL";
     $query = $this->db->query($SQL);
     return $query->result_array();
           }
   ...
   $SQL=   "SELECT * FROM schoolnew_renewal 
        ...
        JOIN schoolnew_block ON schoolnew_block.id=schoolnew_basicinfo.block_id
        join schoolnew_school_department ON schoolnew_school_department.id=schoolnew_basicinfo.sch_directorate_id 
        WHERE schoolnew_basicinfo.block_id=".$block_id." AND (department like '%Directorate%of%Elementary%Education%') AND schoolnew_renewal.id NOT IN (SELECT renewal_id FROM schoolnew_renewapprove) AND schoolnew_renewal.vaild_upto IS NULL";
            $query = $this->db->query($SQL);
        return $query->result_array();
...
if(!empty($school_type))
 ...
 public function govt_enrollment($block_id,$beo_id)
          {
        $SQL="SELECT count(*) as total FROM students_child_detail
JOIN schoolnew_basicinfo ON schoolnew_basicinfo.school_id=students_child_detail.school_id
 WHERE schoolnew_basicinfo.block_id=".$block_id." AND schoolnew_basicinfo.beo_map=".$beo_id."  AND class_studying_id in (13,14,15,1)  AND  doj BETWEEN '2019-03-01' AND NOW() AND created_at BETWEEN '2019-03-01 00:00:00' AND NOW() AND schoolnew_basicinfo.manage_cate_id=1"; 
//print_r($SQL);
//die();
        $query = $this->db->query($SQL);
        return $query->result();
...
{
  $SQL="SELECT * FROM schoolnew_committee_detail JOIN students_school_child_count ON students_school_child_count.school_id=schoolnew_committee_detail.school_key_id WHERE students_school_child_count.district_id=".$block_id." GROUP BY school_id";
       ...
       return $query->result(); 
        ...
        public function uniform_indentboy($scheme,$set,$blk,$school,$class)
	   {
			if(!empty($set))
			{
			$s = "and sets=".$set."";
			...
			$where  = "group by school_name order by school_name asc";
		
		$sql="select  students_school_child_count.school_name,schoolnew_schemeindent.scheme_id,sets,hill_frst,
				...
				SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id  THEN CASE WHEN students_child_detail.gender=1 and schoolnew_schemedetail.scheme_category = 'SIZE I' THEN 1 ELSE 0 END END ) AS bset1_count,
				SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=1 and schoolnew_schemedetail.scheme_category = 'SIZE II' THEN 1 ELSE 0 END END ) AS bset2_count,
				SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=1 and schoolnew_schemedetail.scheme_category ='SIZE III' THEN 1 ELSE 0 END END ) AS bset3_count,
				SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=1 and schoolnew_schemedetail.scheme_category= 'SIZE IV' THEN 1 ELSE 0 END END ) AS bset4_count,
				SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=1 and schoolnew_schemedetail.scheme_category= 'SIZE V' THEN 1 ELSE 0 END END ) AS bset5_count,
				SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=1 and schoolnew_schemedetail.scheme_category = 'SIZE LARGE PRIMARY' THEN 1 ELSE 0 END END ) AS bset6_count,
				SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=1 and schoolnew_schemedetail.scheme_category='SIZE VI' THEN 1 ELSE 0 END END ) AS bset7_count,
				SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=1 and schoolnew_schemedetail.scheme_category = 'SIZE VII' THEN 1 ELSE 0 END END ) AS bset8_count,
				SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id  THEN CASE WHEN students_child_detail.gender=1 and schoolnew_schemedetail.scheme_category ='SIZE VIII' THEN 1 ELSE 0 END END ) AS bset9_count,
				SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=1 and schoolnew_schemedetail.scheme_category = 'SIZE LARGE UPPER PRIMARY' THEN 1 ELSE 0 END END ) AS bset10_count
				from  students_child_detail
	            ...
	            where  schoolnew_schemeindent.scheme_id =1  and  class_studying_id >= 1 and  class_studying_id <= 8  ".$s." and distribution_date is null and transfer_flag =0 and  students_school_child_count.block_id = ".$blk." and schoolnew_schemeindent.isactive =1  ".$schol." and ( school_type_id =1 or school_type_id =2 ) ".$class." ".$where ." ";
                $query = $this->db->query($sql);
	              // print_r($this->db->last_query());
			    return $query->result();
				// join schoolnew_basicinfo on schoolnew_basicinfo.school_id = students_child_detail.school_id
				// join emis_userlogin on emis_userlogin.emis_user_id  =  schoolnew_basicinfo.block_id
	    }
		
		public function uniform_indentgirl($scheme,$set,$blk,$school,$class)
	    {
	       ...
	       $sql="Select  students_school_child_count.school_name,schoolnew_schemeindent.scheme_id,sets,hill_frst,
			...
			SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id  THEN CASE WHEN students_child_detail.gender=2 THEN 1 ELSE 0 END END ) AS girls_count,
			SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=2 and schoolnew_schemedetail.scheme_category = 'SIZE I' THEN 1 ELSE 0 END END ) AS gset1_count, 
			SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=2 and schoolnew_schemedetail.scheme_category = 'SIZE II' THEN 1 ELSE 0 END END ) AS gset2_count,
			SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=2 and schoolnew_schemedetail.scheme_category ='SIZE III' THEN 1 ELSE 0 END END ) AS gset3_count,
			SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=2 and schoolnew_schemedetail.scheme_category = 'SIZE IV' THEN 1 ELSE 0 END END ) AS gset4_count,
			SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=2 and schoolnew_schemedetail.scheme_category = 'SIZE V' THEN 1 ELSE 0 END END ) AS gset5_count,
			SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=2 and schoolnew_schemedetail.scheme_category = 'SIZE LARGE PRIMARY' THEN 1 ELSE 0 END END ) AS gset6_count,
			SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=2 and schoolnew_schemedetail.scheme_category ='SIZE VI' THEN 1 ELSE 0 END END ) AS gset7_count,
			SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=2 and schoolnew_schemedetail.scheme_category = 'SIZE VII' THEN 1 ELSE 0 END END ) AS gset8_count,
			SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=2 and schoolnew_schemedetail.scheme_category ='SIZE VIII' THEN 1 ELSE 0 END END ) AS gset9_count,
			SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=2 and schoolnew_schemedetail.scheme_category = 'SIZE LARGE UPPER PRIMARY' THEN 1 ELSE 0 END END ) AS gset10_count,

			SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id THEN CASE WHEN students_child_detail.gender=3 THEN 1 ELSE 0 END END ) AS trans_count
			 from  students_child_detail
            JOIN   schoolnew_schemeindent ON  schoolnew_schemeindent.student_id = students_child_detail.id and schoolnew_schemeindent.school_id =students_child_detail.school_id
			JOIN students_school_child_count ON students_school_child_count.school_id=students_child_detail.school_id
			join schoolnew_academic_detail on schoolnew_academic_detail.school_key_id =students_child_detail.school_id
            ...
            where  schoolnew_schemeindent.scheme_id =1 and  class_studying_id >= 1 and  class_studying_id <= 8  ".$s." and distribution_date is null and transfer_flag =0 and  students_school_child_count.block_id = ".$blk."  ".$schol." and ( school_type_id =1 or school_type_id =2 )and schoolnew_schemeindent.isactive =1  ".$class." ".$where ." ";
         // echo $sql; join schoolnew_basicinfo on schoolnew_basicinfo.school_id = students_child_detail.school_id
			//join emis_userlogin on emis_userlogin.emis_user_id  =  schoolnew_basicinfo.block_id
		    $query = $this->db->query($sql);
			return $query->result();
	}
	
	public function uniform($scheme,$set,$blk,$school,$class)
	{
		    if($school ==1){
			$schol ="ON schoolnew_academic_detail.hill_frst IN(".$school.")";
			}
			else{ 
			$schol ="ON schoolnew_academic_detail.hill_frst IN (2,3,4,5)";
			}
			if(!empty($set))
			{
			$s = "and sets=".$set."";
		...
		$sql="SELECT 
				...
				schoolnew_basicinfo.school_id,
				class_studying_id
				FROM schoolnew_schemeindent
				JOIN schoolnew_schemedetail ON schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id AND schoolnew_schemedetail.id=schoolnew_schemeindent.scheme_category
				JOIN students_child_detail ON students_child_detail.id=schoolnew_schemeindent.student_id AND students_child_detail.class_studying_id<9
				JOIN schoolnew_basicinfo ON schoolnew_basicinfo.school_id=students_child_detail.school_id AND schoolnew_basicinfo.manage_cate_id IN (1,2) AND curr_stat=1
				JOIN schoolnew_academic_detail ".$schol." AND schoolnew_academic_detail.school_key_id=schoolnew_basicinfo.school_id
				WHERE schoolnew_basicinfo.block_id=".$blk." AND schoolnew_basicinfo.beo_map=1
				GROUP BY schoolnew_basicinfo.school_id,schoolnew_schemedetail.id,class_studying_id";
			$query = $this->db->query($sql);
			return $query->result();
	}
	
		public function footwear_indentboy($scheme,$blk,$class)
	{  
		...
		JOIN students_school_child_count ON students_school_child_count.school_id=students_child_detail.school_id
		JOIN schoolnew_schemedetail ON schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id AND schoolnew_schemedetail.id=schoolnew_schemeindent.scheme_category
		where  schoolnew_schemeindent.scheme_id=".$scheme."  and  class_studying_id >= 1 and  class_studying_id <= 8  and distribution_date is null and schoolnew_schemeindent.isactive =1 and transfer_flag =0 and ( school_type_id =1 or school_type_id =2 )  ".$class."  ".$where." ";
		$query = $this->db->query($sql);
         // print_r($this->db->last_query());
		return $query->result();
	}
	
	public function footwear_indentsgirl($scheme,$blk,$class)
	{
		...
		$sql="select ".$d." schoolnew_schemeindent.scheme_id,
         SUM(CASE WHEN schoolnew_schemeindent.student_id=students_child_detail.id and schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id  THEN CASE WHEN students_child_detail.gender=2 and schoolnew_schemedetail.scheme_category = 'Cat-I:Size 9' THEN 1 ELSE 0 END END ) AS gset1_count,
           ...
           JOIN   schoolnew_schemeindent ON  schoolnew_schemeindent.student_id = students_child_detail.id and      schoolnew_schemeindent.school_id =students_child_detail.school_id
			JOIN students_school_child_count ON students_school_child_count.school_id=students_child_detail.school_id
			JOIN schoolnew_schemedetail ON schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id AND schoolnew_schemedetail.id=schoolnew_schemeindent.scheme_category
			where  schoolnew_schemeindent.scheme_id=".$scheme."  and  class_studying_id >= 1 and  class_studying_id <= 8  and distribution_date is null and schoolnew_schemeindent.isactive =1  and transfer_flag =0  and ( school_type_id =1 or school_type_id =2 ) 
			 ".$class." ".$where."";
	...
	public function bag_indent($class,$blk)
	{
       ...
       where  ( school_type_id =1 or school_type_id =2 )  ".$class."     ".$where." ";
	  ...
	  public function crayan_indent($blk,$class)
	  {
		...
		$sql="select ".$d." 
		 ...
		 from students_child_detail 
		 JOIN students_school_child_count ON students_school_child_count.school_id=students_child_detail.school_id
		 where  class_studying_id >= 1 and class_studying_id <= 2  and ( school_type_id =1 or school_type_id =2 )  ".$class."  ".$where." ";
	 ...
	 public function colorpencil_indent($blk,$class)
	 {
		...
		$sql="select ".$d." 
		 ...
		 from students_child_detail 
		 JOIN students_school_child_count ON students_school_child_count.school_id=students_child_detail.school_id
		 where  class_studying_id >= 3 and class_studying_id <= 5  ".$class."  and ( school_type_id =1 or school_type_id =2 )  
		  ".$where." ";
	    ...
	    public function geomentry_indent($blk,$class)
		{
		...
		public function atlas_indent($blk,$class)
		{
		...
		public function sweater_indent($scheme,$blk,$class,$size)
	 {
       ...
       join schoolnew_schemedetail on schoolnew_schemedetail.scheme_id=schoolnew_schemeindent.scheme_id AND schoolnew_schemedetail.id=schoolnew_schemeindent.scheme_category
	   join schoolnew_academic_detail on schoolnew_academic_detail.school_key_id =students_child_detail.school_id 
       where  distribution_date is null and schoolnew_schemeindent.scheme_id= ".$scheme." and  class_studying_id >= 1 and  class_studying_id <= 8 and hill_frst =1 and schoolnew_schemeindent.isactive =1  and transfer_flag =0 and ( school_type_id =1 or school_type_id =2 )  ".$class ."  ".$sizes." 
	    ".$where." ";
...
// echo $sql;
	   ...
	   public function ankleboot_indent($scheme,$blk,$class)
	  {
		 $d="school_name,";
		$where ="and students_school_child_count.block_id =".$blk." group by school_name order by school_name asc";
		$sql = "select schoolnew_schemeindent.scheme_id,".$d."
		...
		where  schoolnew_schemeindent.scheme_id= ".$scheme."  and  class_studying_id >= 1 and  class_studying_id <= 8  and distribution_date is null  and transfer_flag =0 and ( school_type_id =1 or school_type_id =2 ) and schoolnew_schemeindent.isactive=1 ".$class." ".$where."";
		 // echo $sql;
	  	  ...
	  	  public function socks_indent($scheme,$blk,$class)
	  {
	   ...
	   public function raincoat_indent($scheme,$blk,$class)
	  {
