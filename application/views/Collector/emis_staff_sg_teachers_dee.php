...
<?php $this->load->view('Collector/header.php'); ?>
                                                  ...
                                                  <div class="portlet box green">
                                                    <div class="portlet-title">
                                                        <div class="caption">
                                                            ...
                                                            <i class="fa fa-globe"></i> Staffs SurplusPost with Person Fixation Teacher Type :SG<span> </span></div>
  ...
  <select  class="form-control"  id="blk" name="blk" required="" onchange="getIndex(this.value)">
						 
						  
                            <option  value="">Select Option *</option> 
                           <?php foreach($blk as $res) {   ?>
						   
                                                          <option   
														  value="<?php echo $res->block_id;?>">
														  <?php echo $res->block_name; ?></option>
                                                          <?php  } ?>
                         </select>  						 
                       </div> 
                  </div> 
                  ...
                  <div class="col-md-4" id="Edn"> 
                       ...
                       <div class="form-group" id="sch_grp">    
                       <input type="text" class="form-control" id="sch" name="sch" placeholder="" required >
						
						 
					   
                       </div>         
					   <input type="hidden" class="form-control" id="manage_sch" name="manage_sch" placeholder="" required>
				   ...
				   <div class="col-md-2"> 
                       ...
                       <div class="form-group"> 
                                                        ...
                                                        <table class="table table-striped table-bordered table-hover" id="sample_2">
                                                            ...
                                                            <thead>
                                                                <tr>
																	...
																	<th class=" ">DOJ Block</th>
																	<th class=" ">Seniority</th>
																	<th class=" " style="display:none" >RANK</th>
																	<th class=" ">Action</th>
																	                                                                                                                         </tr>
                                                                </thead>
                                                               <tbody>
														   ...
														   if(!empty($sgteachers)){ $total1 = 0; foreach($sgteachers as $sd){   ?>
                                                                ...
                                                                <tr>
                                                                 ...
                                                                 <td><?php echo $sd->teacher_id;?>
																 <input type ="hidden" value ="<?= $sd->teacher_id;?>" id= "teach<?=$i?>" class ="teach">
																 <input type ="hidden" value ="<?= $sd->school_id;?>" id= "school<?=$i?>" class ="school"></td>
																
																 <td><?php echo $sd->subjects?></td>
																  <td><?php echo  date('d-m-Y', strtotime($sd->staff_dob))?></td>
																  <td><?php echo  date('d-m-Y', strtotime($sd->staff_join))?></td>
																	<td><?php echo  date('d-m-Y', strtotime($sd->staff_pjoin))?></td>
																	<td><?php echo  date('d-m-Y', strtotime($sd->staff_psjoin))?></td> 
                                                                  <!-- <td><input type="text" class="form-control" id="number<?php //echo $i;?>"  onKeypress="return isNumberKey(event);" value="<?php //echo $sd->seniority_district?>"></td> 
						...
						<td><?= date('d-m-Y', strtotime($sd->doj_block)) == '' ||date('d-m-Y', strtotime($sd->doj_block)) =='01-01-1970'? ' ': date('d-m-Y', strtotime($sd->doj_block)) ?></td>



																	
																	<td><input type="text" class="form-control" id="rank<?php echo $i;?>"value ="<?php echo $sd->seniority_district?>">

																	</td>
<td style="display:none" ><?php echo $sd->seniority_district?></td>																	
																	
																	<td>
																	
																	
																	<button type="submit" class="btn green btn-lg" onclick ="save(<?php echo $i;?>)">Submit</button></td>
...
// {
   ...
   // function save(i,teachercode)  
		 // {
		   ...
		   // teacher_code=teachercode;
			...
			// url:baseurl+'Collector/emis_teacher_surplus_add_dee',
			// type:"POST",
			// dataType:"JSON",
		 ...
		 function save(i)  
		 {
		   ...
		   // alert(school);
		    ...
		    $.ajax(
		    {
			data:{'teach':teach,'school':school,'rank':rank},
			
			url:baseurl+'Collector/save_surplus_rank',
			type:"POST",
			dataType:"JSON",
			success:function(res)
			{
 ...
 function isNumberKey(evt){
    ...
    return true;
		...
		var stafftranschool = <?php echo json_encode($blk);?>;
...
function getIndex(sa)
		{
			...
			var staffin = stafftranschool.filter(stu=>stu.block_id == sa)[0];
			console.log(staffin);
			var sch_drop = '';
			if(staffin.municipal_sch !=null && staffin.panchayat_sch !=null)
			{
				$("#Edn").show();
				sch_drop +='<select class="form-control" name="manage_sch" id="manage_sch">';
				sch_drop +='<option value="1">Municipal School</option>';
				sch_drop +='<option value="3">Panchayat School</option></select>';
			}else
			{
				$("#Edn").hide();
				$("#manage_sch").val(staffin.municipal_sch !=null?'1':'3');
			}
			$("#sch_grp").html(sch_drop);
		    
			$("#sch1").val(staffin.block_id);
			// $("#blk").val(staffin.block_name);
