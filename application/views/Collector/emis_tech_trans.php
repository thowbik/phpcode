...
<link href="<?php echo base_url().'asset/global/plugins/bootstrap-datepicker/css/bootstrap-datepicker3.min.css';?>" rel="stylesheet" type="text/css" />
    ...
    <?php  
     $this->load->view('Collector/header.php');?>
 ...
 // });
  ...
  function transferteacher()  
		...
		if(category=='Deputation')
		{
			 ...
			 $.ajax(
		            {
			data:{'old_school':old_school,'new_school':new_school,'oldschool_key':oldschool_key,'newschool_key':newschool_key,'teacheruid':teacheruid,'transferdate':transferdate,'new_dist':new_dist,'old_dist':old_dist,'new_block':new_block,'old_block':old_block,'transfer_by':transfer_by,'category':category},
			url:baseurl+'Collector/transfer_teacher_debutation',
			type:"POST",
			dataType:"JSON",
		...
		else
		{
				 ...
				 $.ajax(
		            {
			data:{'old_school':old_school,'new_school':new_school,'oldschool_key':oldschool_key,'newschool_key':newschool_key,'teacheruid':teacheruid,'transferdate':transferdate,'new_dist':new_dist,'old_dist':old_dist,'new_block':new_block,'old_block':old_block,'transfer_by':transfer_by,'category':category},
			url:baseurl+'Collector/transfer_teacher',
			type:"POST",
			dataType:"JSON",
			success:function(res)
			{
