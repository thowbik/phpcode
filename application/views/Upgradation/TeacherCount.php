                                        ...
                                        <h1>Dashboard - <label style="color:red;"><?php echo $details[0]->district_name;?></label>
                                          ...
                                          <ul id="myTab" class="nav nav-tabs nav-tabs-responsive" role="tablist">
                                            ...
                                            <a href="<?php echo base_url().'AllApprove/teacherCount/1'?>">
                                              <span class="text">Teaching Staff</span><br/>
                                            </a>
                                          </li>
                                           <li role="presentation">
                                            <a href="<?php echo base_url().'AllApprove/teacherCount/2'?>">
                                              <span class="text">Non Teaching Staff</span><br/>
                                            </a>
                                          </li>
                                           <li role="presentation">
                                            <a href="<?php echo base_url().'AllApprove/teacherCount/3'?>">
                                              <span class="text">Government Teaching Staff By Grade</span>
                                            </a>
                                          </li>
                                          </li>
                                                </ul>
                                                    ...
                                                    <table class="table table-striped table-bordered table-hover" id="sample_2">
                                                        <thead>
                                                            <tr>
                                                                ...
                                                                ?>
                                                                    <th><?php print_r($m['manage_name']); array_push($mag,$m['manage_name']); ?></th>
                                                                <?php
                                                                            }
                                                                       }
                                                        ...
                                                        <tbody>
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td><?php if($group['next']!=''){ ?><a href="javascript:null;" onclick="seturlpost(this)" url="<?php echo base_url().'AllApprove/teacherCount/'.$this->uri->segment(3,0).'/'.$grp[0][$group['group']].'/'.$group['next'];?>"><?php } echo($grp[0][$group['groupname']]); ?></a>
                                                                <?php
                                                                    $discon='';
                                                                    ...
                                                                    elseif($this->uri->segment(3,0)==3){
                                                                        $discon=['hm','bt','pg','sg','oth'];
                                                                    }
                                                                    //$discon=(($this->uri->segment(3,0)==1)?'teach_total':($this->uri->segment(3,0)==2)?'nonteach':($this->uri->segment(3,0)==3)?['hm','bt','pg','sg','oth']:'');
                                                                    if(is_array($discon)){
                                                                        $i=0;
                                                                       foreach($grp as $g){
                                                                            foreach($discon as $d){
                                                                    ?>
                                                                        <td><?php echo($g[$d]); $total[$i]+=$g[$d];$i++; ?></td>
                                                                    <?php
                                                                            }
                                                                       }
                                                                    }
                                                                    elseif($discon!=''){ 
                                                                        $teach = array();$key='school_type';
                                                                        foreach($grp as $val) {
                                                                            if(array_key_exists($key, $val)){
                                                                                $teach[$val[$key]][] = $val;
                                                                            }else{
                                                                        ...
                                                                        foreach($teach as $t){ 
                                                                ?>
                                                                <td><?php echo($t[0][$discon]); $lstkey++; $total[$i]+=$t[0][$discon];$i++; $sum+=$t[0][$discon]; ?></td>
                                                                <?php
                                                                        }
                                                                        for($zz=$lstkey;$zz<=count($mag);$zz++){
                                                                            $total[$i]+=0;$i++;
                                                                ?>
                                                                <td>0</td>
                                                                <?php
