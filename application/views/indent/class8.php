...
<tr>
  <th colspan="17">Class-VIII &nbsp;&nbsp;  No Of Students </th>
</tr>
  <tr>
    ...
    <th colspan=7>No of NoteBooks</th> 
  ...
  <tr>
    ...
    <td style="color:#00A65A;">Graph</td>
...
<select {% if i.nutritious_meal_flag == '0' %}title="{{ i.name }} did not eat noon meal, so this student not eligible for uniform" {% endif %} style="width:100px;"  name='uniform_{{forloop.counter}}' class="form-control" >
  ...
  {% else %}
  {% for u in us %}
  <option {% if student_schemes_list_8  and us8.id == u.id %}  selected  {% endif %} value="{{u.id}}"  >{{u}} </option>
  {% endfor %}
  {% endif %}
</select>
</td>
...
<select style="width:100px;" name='footware_{{forloop.counter}}' class="form-control" >
  {% for f in fs %}
  <option {% if student_schemes_list_8  and fs8.id == f.id %}  selected  {% endif %} value="{{f.id}}" >{{f}} </option>
  {% endfor %}
</select>
</td>
...
<select style="width:100px;"  name='bag_{{forloop.counter}}' class="form-control" >
  {% for b in bs %}
  <option {% if student_schemes_list_8  and bs_lr.id == b.id %}  selected  {% endif %} value="{{b.id}}"  >{{b}} </option>
  {% endfor %}
</select>
</td>
...
<td>
<input  class="form-control" size="1" readonly name='graph_{{forloop.counter}}'  value=1>
</td>
<td>
