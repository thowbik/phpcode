...
<tr>
	<th colspan="17">Class-I &nbsp;&nbsp;  No Of Students-{{ student_schemes_list_1|length }} </th>
</tr>
	<tr>
...
<select {% if i.nutritious_meal_flag == '0' %}title="{{ i.name }} did not eat noon meal, so this student not eligible for uniform" {% endif %} style="width:100px;"  name='uniform_{{forloop.counter}}' class="form-control" >
  ...
  {% else %}
  {% for u in us %}
  <option  {% if student_schemes_list_1  and us1.id == u.id %}  selected  {% endif %} value="{{u.id}}"  >{{u}} </option>
  {% endfor %}
  {% endif %}
</select>
</td>
...
<select style="width:100px;" name='footware_{{forloop.counter}}' class="form-control" >
  {% for f in fs %}
  <option {% if student_schemes_list_1  and fs1.id == f.id %}  selected  {% endif %} value="{{f.id}}" >{{f}} </option>
  {% endfor %}
</select>
</td>
...
<select style="width:100px;"  name='bag_{{forloop.counter}}' class="form-control" >
  {% for b in bs %}
  <option {% if student_schemes_list_1  and bs_sm.id == b.id %}  selected  {% endif %} value="{{b.id}}"  >{{b}} </option>
  {% endfor %}
</select>
</td>
...
<select style="width:100px;"  name='sweater_{{forloop.counter}}' class="form-control" >
  {% for s in ss %}
  <option {% if student_schemes_list_1  and ss_sm.id == s.id %} %} selected {% endif %} value="{{s.id}}"  >{{s}} </option>
  {% endfor %}
</select>
</td>
