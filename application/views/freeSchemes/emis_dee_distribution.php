...
<html lang="en">
    ...
    <body class="page-container-bg-solid page-md">
                            ...
                            <div class="page-head">
                                <div class="container">
                                    ...
                                    <div class="page-title"  id="page-title">
                                        <h1> DEE <?php echo($title); ?> Wise - Distribution List </h1>
                                        <small id="breadcrumbs">
                                                                        <?php if($providing_list != null){
                                                                                foreach($providing_list as $bread){
                                                                                     $breadcrumbs='';
                                                                                     $crumbs_district = '<a href="'.base_url().'Basic/dee_providing_list/0/0/'.$this->uri->segment(5,0).'/'.$this->uri->segment(6,0).'">District</a> &GreaterGreater;';
                                                                                     $crumbs_edudistrict = '<a href="'.base_url().'Basic/dee_providing_list/EDU/'.$bread->district_id.'/'.$this->uri->segment(5,0).'/'.$this->uri->segment(6,0).'">Educational District</a> &GreaterGreater;';
                                                                                     $crumbs_block = '<a href="'.base_url().'Basic/dee_providing_list/Block/'.$bread->edu_dist_id.'/'.$this->uri->segment(5,0).'/'.$this->uri->segment(6,0).'">Block</a> &GreaterGreater;';
                                                    ...
                                                    <div class="page-content-inner">
                                                        ...
                                                        <div class="portlet light portlet-fit ">
                                                            ...
                                                            <?php } $user_type_id = $this->session->userdata('emis_user_type_id'); 
                                                                    ...
                                                                    $max_class = $max_class >=9 ? 8 : $max_class; 
                                                            ?> 
                                                                    ...
                                                                    <form id="text_form_id" name="text_form_id" method="post" action="<?php echo $_SERVER['REQUEST_URI']; ?>">
                                                                                <table class="table table-striped">
                                                                                    <tr>
                                                                                        ...
                                                                                        <th id="th_term" style="width:300px;">
                                                                                            <label class="control-label bold">Term</label>
                                                                                            <select class="form-control"  id="term_name" name="term" onchange="changebookslist()">
                                                                                            <option value="">Choose Term</option>
                                                                                            <?php for($i=1;$i<=3;$i++){ ?>
                                                                                                <option value="<?php echo $i; ?>"  <?php if($termid==$i){echo('selected');} ?> ><?php echo 'Term '.$i ?></option>  
                                                                                            <?php } ?>
                                                                                            </select> 
                                                                                        </th>
                                                                                        <th id="th_book" style="width:300px;">
                                                                                            <label class="control-label bold">Books</label>
                                                                                            <select class="form-control"  id="book_name" name="bkname">
                                                                                                <option value="">Choose Books</option>
                                                                                                <?php foreach($textbooklist as $B){ ?>
                                                                                                        <option value="<?php echo($B->book_id); ?>"  <?php if($bookid==$B->book_id){echo('selected');} ?> ><?php echo($B->book_name); ?></option>
                                                                                                <?php } ?>
                                                                                            </select> 
                                                                                        </th>
                                                        ...
                                                        <div class="portlet box green">
                                                            ...
                                                            <div class="portlet-body">
                                                                ...
                                                                <table class="table table-striped table-bordered table-hover" id="sample_3" style="text-align: center;">
                                                                                                ...
                                                                                                <tbody>
                                                                                                <?php $sno=1; 
                                                                                                            if($schemeid == 9){ $concat_id = $this->input->post('term').'-'.$this->input->post('bkname'); }
                                                                                                            foreach($providing_list as $d){  
                                                                                                                $sid = $schemeid != null ? $schemeid : 0;
                                                                                                                if($schemeid == 9) $dyn = $this->uri->segment(5,0)!=0 ? $this->uri->segment(5,0) : $d->mediumid.'-'.$concat_id;
                                                                                                                else if($schemeid == 1) $dyn = $this->uri->segment(5,0)!=0 ? $this->uri->segment(5,0) : $setid;
                                                                                                                else $dyn = 0;
                                                                                                            ?>
                                                                                                    ...
                                                                                                    <tr>
                                                                                                        ...
                                                                                                        <td style="text-align: left;">
                                                                                                            <?php if($next!=''){ ?>
                                                                                                              <a href="<?php echo(base_url().'Basic/dee_providing_list/'.$next.'/'.$d->$idval.'/'.$dyn.'/'.$sid); ?>"> 
                                                                                                            <?php echo($d-> $index); ?></a><?php } else{echo($d -> $index);} ?></td>
                                                                                                            <?php for($i=$min_class;$i<=$max_class;$i++){
                                                                                                                    $class_des = 'Class'.$i.'_Despatched';
                                                                                                                    $class_eli = 'Class'.$i.'_Eligible';
                                                                                                                    $class_dis = 'Class'.$i.'_Distributed';  ?>
                                                                                                                    <td style="text-align: center;"><?php echo $d->$class_eli; ?></td>
                                                                                                                    <td style="text-align: center;"><?php echo $d->$class_des; ?></td>
                                                                                                                    <td style="text-align: center;"><?php echo $d->$class_dis; ?></td>
                                                                                                            <?php } ?>
                                                                                                     </tr>
                                                                                                    <?php } ?>
                                                                                                 </tbody>
        ...
        function changebookslist(){
            var term = $('#term_name').val();
            var medium = $('#medium_name').val();
            if(term != '' && medium != ''){ $("#th_book").show();
                $('#book_name').empty().append('<option value="">Choose Books</option>');
                var pausedropdowncontent = <?php echo json_encode($textbooklist); ?>;
                pausedropdowncontent.filter(function(val) {
                    if(val.medium == medium && val.term == term) {    
                        $('#book_name').append($('<option></option>').text(val.book_name).attr('value', val.book_id));
                    }
                });
             }
            else{ 
                $("#th_book").hide(); $('#book_name').val('');
            }
        }
        ...
        $('#dee_rpt_btn_submit').click(function(){
            ...
            if($("option:selected", $("#scheme_name")).val() == 9){
                ...
                if($("option:selected", $("#book_name")).val() == "")
                {
        ...
        function sum_dataTable(dataId,x_status){
            var table = $(dataId).DataTable({
                ...
                "buttons": [
                        ...
                        { extend: 'csv', className: 'btn default' ,filename: function(){ return 'EMIS TN Schools DEE <?php echo($title); ?> Wise - <?php echo ($selectedSchemeName)?> Distribution List';}}                                                                           
                ],          
