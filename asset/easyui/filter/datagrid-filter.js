(function ($) {
    ...
    var updateMethod1 = $.fn.datagrid.methods.updateRow;
    $.extend($.fn.datagrid.methods, {
        ...
        loadData: function (jq, data) {
            jq.each(function () {
                ...
                var cells = $(this).datagrid('getPanel').find('div.datagrid-body td[field] div.datagrid-cell');
        ...
        deleteRow: function (jq, index) {
            jq.each(function () {
                ...
                if (opts.oldLoadFilter && opts.idField) {
                    for (var i = 0; i < state.filterSource.rows.length; i++) {
                        var row = state.filterSource.rows[i];
                        var rowValue = parseInt(row[opts.idField]);
                        if (rowValue === parseInt(index)) {
                            var loginName = $.jStorage.get($.i18n.prop('LOGGED_IN_USER_NAME_KEY'));
                            if (row.hasOwnProperty("ackfield") && (row.ackfield === 2 || row.ackfield === 3) && loginName === row.ackedby) {
                                row['ackfield'] = 3;
                                var newRows = [];
                                newRows = newRows.concat(state.filterSource.rows);
                                newRows.splice(i, 1, row);
                                state.filterSource.rows = newRows;
                                //state.filterSource.rows[i].ackfield = 3;
                            } else {
                    ...
                    $(this).datagrid('loadData', state.filterSource);
                } else {
                    deleteMethod1.call($.fn.datagrid.methods, jq, index);
                }
            });
        },
        ...
        insertRow: function (jq, row) {
            jq.each(function () {
                ...
                if (opts.oldLoadFilter && opts.idField) {
                    state.filterSource.total++;
                    var newRows = [];
                    newRows = newRows.concat(row.row);
                    newRows = newRows.concat(state.filterSource.rows);
                    state.filterSource.rows = newRows;
                    $(this).datagrid('loadData', state.filterSource);
                } else {
                    insertMethod1.call($.fn.datagrid.methods, jq, row);
                }
            });
        },
        updateRow: function (jq, row) {
            jq.each(function () {
                ...
                if (opts.oldLoadFilter && opts.idField) {
                    for (var i = 0; i < state.filterSource.rows.length; i++) {
                        var row1 = state.filterSource.rows[i];
                        var rowValue = parseInt(row1[opts.idField]);
                        if (rowValue === parseInt(row.index)) {
                            $.each(row.row, function (key, value) {
                                state.filterSource.rows[i][key] = value;
                            });
                            break;
                        }
                    }
                    ...
                    updateMethod1.call($.fn.datagrid.methods, jq, row);
                }
            });
        },
        getRow: function (jq, recordId) {
            var selRow = null;
            jq.each(function () {
                var state = $(this).data('datagrid');
                var opts = state.options;
    ...
    $.fn.datagrid.defaults.operators = {
        ...
        custom: {
            ...
            isMatch: function (source, value) {
                ...
                if (value.indexOf("!") >= 0) {
                    valueRegExp = new RegExp(value.replace("!", ""), 'gi');
                    return !source.match(valueRegExp);
                }
                return source.match(valueRegExp);
            }
        },
            ...
            if (opts.pagination) {
                ...
                pager.pagination({
                    onSelectPage: function (pageNum, pageSize) {
                        /* ifdef CUSTOM_CHANGE */
                        var cells = $(target).datagrid('getPanel').find('div.datagrid-body td[field] div.datagrid-cell');
                        cells.tooltip('hide');
                        $(pager).find('a.l-btn').each(function () {
                            try {
                                $(this).tooltip('hide');
    ...
    function init(target, filters) {
        ...
        for (var i = 0; i < opts.filterRules.length; i++) {
            addFilterRule(target, opts.filterRules[i]);
        }
