...
SELECT renewal_id,academic_yr,
	...
	pending_with,
	IF(pending_with IS NULL AND pending=6,'NO BEO MAPPED',pending_with) AS pending_desc,username_pass
FROM
	(SELECT renewal_id,
		...
		IF(pending IS NOT NULL AND (pending!='APPROVED' AND pending!='REJECTED'),
			(SELECT UPPER(user_category.user_type) 
				FROM emis_userlogin 
			JOIN user_category ON user_category.id=emis_userlogin.emis_usertype
			WHERE emis_user_id=IF(pending IN (9,12,13),district_id,IF(pending IN (6),block_id,IF(pending IN (10),edu_dist_id,0))) 
            AND emis_usertype=pending AND IF(pending IN (6) AND applied_category!=4,emis_usertype1=beo_map,emis_usertype1 IS NOT NULL) 
            AND status='Active' LIMIT 1),IF(pending='REJECTED','REJECTED',IF(pending='APPROVED','APPROVED',directorate))) AS pending_with,
IF(pending IS NOT NULL AND (pending!='APPROVED' AND pending!='REJECTED'),(SELECT CONCAT(emis_username," / ",ref) 
FROM 
emis_userlogin 
JOIN user_category ON user_category.id=emis_userlogin.emis_usertype
WHERE emis_user_id=IF(pending IN (9,12,13),district_id,IF(pending IN (6),block_id,IF(pending IN (10),edu_dist_id,0))) AND emis_usertype=pending AND IF(pending IN (6),emis_usertype1=beo_map,emis_usertype1 IS NOT NULL) AND status='Active' LIMIT 1),IF(pending='REJECTED','REJECTED',IF(pending='APPROVED','APPROVED',directorate))) AS username_pass
FROM 
(SELECT 
    ...
    UPPER((SELECT department FROM schoolnew_school_department WHERE id=renewal.sch_directorate_id)) AS directorate,
...
JOIN schoolnew_renewcategory FORCE INDEX(renewal_id) ON schoolnew_renewcategory.renewal_id=schoolnew_renewal.id AND schoolnew_renewcategory.applied_category<>4
