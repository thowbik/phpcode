...
class Common
{
	...
	***/
	...
	public static function get_multi_withdyncol_list($param,$table,$where)
	{
			$CI = & get_instance();
			$CI->load->database();
			$CI->db->from($table);
	...
	public static function get_single_withdyncol_list($param,$table,$where)
	{
			$CI = & get_instance();
			$CI->load->database();
			// $CI->db->where($where);
		$result = $CI->db->select($param)->get_where($table,$where)->first_row();
		// print_r($CI->db->last_query());
		return $result;
