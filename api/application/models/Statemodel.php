...
public function Get_BRTE_TeachersList($dist){
  ...
  //print_r($this->db->last_query());die();
  $sql = "select udise_staffreg.u_id as brte_id,udise_staffreg.teacher_name as brte_name from udise_staffreg join udise_offreg on udise_staffreg.school_key_id = udise_offreg.off_key_id where udise_staffreg.teacher_type = 103 and udise_staffreg.archive = 1 and udise_offreg.district_id = $dist;";
  $query = $this->db->query($sql);
  $result = $query->result_array(); 
  return $result;
}
...
public function Get_PLAStdAttentRep($district_id,$block_id,$date)
{  
    $sql = "select sb.block_name as Block,sb.udise_code as UDISE,sb.school_name as SclName,tv.teacher_name as Volunteer_Name, 
    (CASE WHEN (snfa.status=1) THEN 'MARKED' ELSE 'NOT MARKED' END) as Marked_Status, case when snfa.volunteer_attendance =1 
    then 'Present' else 'Absent' end as VolunteerAttend, snfa.allp as Learners_Present,snfa.allA as Learners_Absent from students_school_child_count sb 
    left join schoolnew_nonformal_attendance snfa on snfa.school_id = sb.school_id and date(attendance_date) = '$date'
    join teacher_volunteers tv on tv.school_key_id = sb.school_id and tv.type>0 and tv.Active=1 ";
    if(!empty($district_id)){
      $sql .= "where district_id=$district_id and school_type_id in(1,2,4) group by sb.school_id,tv.id;";
    }else if(!empty($block_id)){
      $sql .= "where block_id=$block_id and school_type_id in(1,2,4) group by sb.school_id,tv.id;";
    }
    $query = $this->db->query($sql);
    $result = $query->result_array();
    return $result;  
}
public function Get_StudDetailsList($records)
{
  $school_id= $records['school_id'];
  $class_id=$records['class_id'];
  $section = $records['section'];
  $sql = "select students_child_detail.id,name,class_studying_id,class_section, school_id from students_child_detail where  class_studying_id=$class_id and class_section='$section'  and school_id=$school_id";
$query = $this->db->query($sql);
$result = $query->result_array(); 
return $result;
}
