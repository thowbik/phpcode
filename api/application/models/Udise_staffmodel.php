...
where udise_staffreg.u_id='.$unique_id_staff;
    ...
    function get_teaching_staff_details($school_id,$teacher_id){
...
where  $where AND  udise_staffreg.archive=1  group by teacher_code";
    //echo($SQL);die();
    $query = $this->db->query($SQL);
...
where udise_staffreg.u_id=".$unique_id_staff;
    ...
    function getvolunteer($school_id,$teacher_id) {
      if(!empty($school_id))
      {
        $where = 'teacher_volunteers.school_key_id='.$school_id.'';
      }
      else if(!empty($teacher_id))
    {
      $where = 'reg.teacher_id='.$teacher_id.'';
     $join = "join udise_staffreg reg ON reg.teacher_code =teacher_volunteers.teacher_code";
    }
    //print_r($school_id);
    ...
    left join teacher_subjects b on b.id=teacher_volunteers_subjects.sub2
     left join teacher_subjects c on c.id=teacher_volunteers_subjects.sub3
       $join where $where group by teacher_volunteers.id";
        $query = $this->db->query($sql);
        return $query->result();
    }
    
    function getdepute($school_id,$teacher_id) {
      if(!empty($school_id))
      { 
        $where = 'teacherdepute_entry.depute_key='.$school_id.'';
      }
      else if(!empty($teacher_id))
      {
        $where = 'udise_staffreg.teacher_id='.$teacher_id.'';
...
WHERE $where AND teacherdepute_entry.deputed_place=1 AND teacherdepute_entry.depute_todate > '".$today."'";
//echo($sql);die();
                $query = $this->db->query($sql);
                return $query->result();
...
//created by vit for update archieve,emis_school_id,emis_school_udise (Admit)//   
  ...
  *****/
  ...
  /****************************************
   ...
   function get_staff_part2_details($school_id,$teacher_id)
  {
   
     if(!empty($school_id))
       {
         $where = 'dates.school_key_id='.$school_id.'';
       }
       else if(!empty($teacher_id))
     {
       $where = 'reg.teacher_id='.$teacher_id.'';
      $join = "join udise_staffreg reg ON reg.teacher_code =dates.teacher_id";
     }

      $sql ="select dates.id,dates.school_key_id,dates.teacher_id,dates.sslc_dop,dates.higersec_dop,dates.doj_block,dates.promo_eligi_date,dates.relinguish,dates.relinguish_date,
      dates.prob_id,dates.prob_date,dates.doj_dept,dates.head_account from  teacher_dates dates $join where $where";
               $query = $this->db->query($sql);
        return $query->result();
        
  }
   function get_staff_part3_details($school_id,$teacher_id)
  {
    
    if(!empty($school_id))
    {
      $where = 'regu_dt.school_key_id='.$school_id.'';
    }
    else if(!empty($teacher_id))
  {
    $where = 'reg.teacher_id='.$teacher_id.'';
   $join = "join udise_staffreg reg ON reg.teacher_code =regu_dt.teacher_id";
  }

      
      $sql ="select regu_dt.id,regu_dt.teacher_uid,regu_dt.teacher_id,regu_dt.school_key_id,regu_dt.type_date,regu_dt.dates,regu_dt.appoint_nature
            from  teacher_regu_dates regu_dt
               $join where $where";
        $query = $this->db->query($sql);
        return $query->result();
...
function delete_nsqf_trainee_staff_dtl($id){
 ...
 where sc.school_type_id in ($school_type) and u.archive = 1 and sc.district_id = $dist and t.category = 1 group by u.u_id having Days is Null or Days = 0";
  ...
  public function teacher_volunteersave($data) {
    if($data['id'] !=''){
      $up=$this->db->update('teacher_volunteers', $data, "id = ".$data['id']);
      unset($data['created_date']);
      unset($data['id']);
      $up=$this->db->insert('teacher_volunteers_log', $data, "id = ".$data['id']);
      return $up;
    } else {
      $test = $this->db->insert('teacher_volunteers',$data);
      if(!empty($test)){
        return 1;
      } else {
        return 0;
  ...
  public function primer_book_save($data)
  {
    ...
    if($data[0]['UsrType'] == 1){
      foreach($data as $dt){
        $stuntData[]= array(
            'id'=>$dt['IndxID'],
            'primer'=>$dt['BookIssued'],
      );
    }
    $res = $this->db->update_batch('students_non_formal', $stuntData, 'id');
    return $res;
    } else {
      foreach($data as $dt){
        $techerData[]= array(
            'id'=>$dt['IndxID'],
            'guidebook'=>$dt['BookIssued'],
      );
    }
    $res = $this->db->update_batch('teacher_volunteers', $techerData, 'id');
    return $res;
    }
  }

  public function del_stu_teac_save($data)
  {
    $getData = $data['UsrType'];
    if($getData == 1){
      $stuntData = array(
        'id' => $data['IndxID'],
        'Active' => '0'
      );
    $up=$this->db->update('students_non_formal', $stuntData, "id = ".$stuntData['id']);
    return $up;
    } else {
      $teacherData = array(
        'id' => $data['IndxID'],
    ...
    $up=$this->db->update('teacher_volunteers', $teacherData, "id = ".$teacherData['id']);
    return $up;
    }
  }

  public function get_teacher_volunteer($school_id) {
    $SQL="select tv.id as IndxID,tv.school_key_id as SchlID,sc.school_name as SchlName,
    tv.teacher_name as TeachNam,tv.teacher_code as TeachCod,
    tv.aadhar_no as Aadhar,tv.email as Email,tv.gender as Gendr,
    tv.staff_dob as StaffDOB,tv.staff_join as StaffJoin,
    tv.organization_type as OrgTyp,tv.organization_name as OrgNam,
    tv.mbl_nmbr as MblNo,tv.social_category as SclCategory,
    tv.type as VolunteerCategory,tv.academic as Academic,
    tv.e_ug as E_UG,tv.e_pg as E_PG,tv.professional as Profsnl,
    tv.age as AGE,Active as Status,tv.guidebook as BookIssued from teacher_volunteers tv join students_school_child_count sc ON sc.school_id = tv.school_key_id
     where school_key_id = $school_id and active = 1";
    $query = $this->db->query($SQL);
    return $query->result_array();
  }

  public function get_school_volunteer_list($brte_id) {
   // echo $block_id.'+'.$brte_id;die();
    if(!empty($brte_id))
    {
      
       $SQL="select brte_id,map_type,schoolnew_brte_map.zonal_id,schoolnew_brte_map.isactive,subschool_id,schoolnew_zonal_subschools.zonal_id as subschoolzonal,schoolnew_zonal_subschools.isactive,schoolnew_basicinfo.school_name,a.school_name as zonalSchoolName,a.school_id, students_school_child_count.total,
    students_school_child_count.totstaff from schoolnew_brte_map
    join schoolnew_zonal_subschools on schoolnew_zonal_subschools.zonal_id=schoolnew_brte_map.zonal_id
    join schoolnew_basicinfo on schoolnew_basicinfo.school_id=schoolnew_zonal_subschools.subschool_id
    join schoolnew_basicinfo as a on a.udise_code=schoolnew_brte_map.zonal_id
    join students_school_child_count on students_school_child_count.school_id=schoolnew_zonal_subschools.subschool_id
    where map_type=2 and schoolnew_brte_map.isactive=1 and brte_id=$brte_id;"; 
    $query = $this->db->query($SQL);
     return $query->result_array();
    }
  ...
  public function get_school_list($block_id,$teacher){
  if(!empty($block_id))
    {
     $sql="select school_id as SchlID,school_name as SchlNam from students_school_child_count where block_id=$block_id";  
    }
    else if(!empty($teacher))
    {
      $sql="select school_id as SchlID,school_name as SchlNam from udise_staffreg  join  students_school_child_count ON students_school_child_count.school_id = udise_staffreg.school_key_id where teacher_id=$teacher";  
...
*************************************************************************************/
  ...
  ********************************************************************************************/
  ...
  public function teacher_panel_details($school_id)
  {
    $SQL="select id,school_key_id,vac_1,vac_2,vac_3,vac_4,vac_5,vac_6,vac_7,vac_8,vac_9,vac_10,vac_11,vac_12,vac_13,vac_14,vac_15,vac_16,vac_17,vac_18,vac_19,vac_20
    ,vac_20,vac_21,vac_22,vac_23,vac_24,vac_25,vac_26,vac_27,vac_28,vac_29,vac_30,vac_31,vac_32,vac_33,vac_34,vac_35,vac_36,vac_37,vac_38,vac_39,vac_40,vac_41,vac_42,vac_43,
    vac_44,vac_45,vac_46,vac_47,vac_48,vac_49,vac_50 from  teacher_panel4 where school_key_id=$school_id";
    $query=$this->db->query($SQL);
    return $query->result_array();
  }
  public function get_Taught_dropdown(){
    $SQL="select * from schoolnew_nonformal_lessons where isactive=1";
    $query=$this->db->query($SQL);
    return $query->result_array();
  }
  public function get_Maintanance_det($school_id,$id)
  {
    if(!empty($id))
    {
      $SQL="select id as IndxID, school_id as SchlID,maintenance_type as MantnceType,problems as Prblms,attachment as IssuePhto,
      quotation as QuotePhto,requested_amount as ReqAmt, remarks as Remrks,status as Stats,payment_status as PayMntStats
      from schoolnew_maintenance where active=1 and id=$id";
    }
    else if(!empty($school_id))
    {
      $SQL="select id as IndxID, school_id as SchlID,maintenance_type as MantnceType,problems as Prblms,attachment as IssuePhto,
      quotation as QuotePhto,requested_amount as ReqAmt, remarks as Remrks,status as Stats,payment_status as PayMntStats
      from schoolnew_maintenance where active=1 and school_id=$school_id";
  ...
  public function get_BilingData_det($school_id,$id)
{
  if(!empty($id))
    {
      $SQL="select id as IndxID, school_id as SchlID,type as Typ,bill_amount as BillAmt,billing_month as BillMonth,billing_year as BillYear,attachment as BillPhoto,
      remarks as Remrks,status as Status, transaction_status as TrnsactnSts,active as DataSts
      from schoolnew_bill_charges where active=1 and id=$id";
    }
    else if(!empty($school_id))
    {
      $SQL="select id as IndxID, school_id as SchlID,type as Typ,bill_amount as BillAmt,billing_month as BillMonth,billing_year as BillYear,attachment as BillPhoto,
      remarks as Remrks,status as Status, transaction_status as TrnsactnSts,active as DataSts
      from schoolnew_bill_charges where active=1 and school_id=$school_id";
  ...
  public function Maintnce_InsrtUpdt($rec)
  { 
   $count=count($rec);
   
  
    $records_details = array('id'=>$rec['IndxID'],'school_id'=>$rec['SchlID'],'maintenance_type'=>$rec['MantnceType'],'problems'=>$rec['Prblms'],
    'attachment'=>$rec['IssuePhto'],'quotation'=>$rec['QuotePhto'],'requested_amount'=>$rec['ReqAmt'],'remarks'=>$rec['Remrks']);
    
    if($count == 1)
    {
      $params = array('active'=>0);
      $this->db->where('id',$records_details['id']);
      $this->db->update('schoolnew_maintenance',$params);
       $affectedRows=$this->db->affected_rows();
       ...
       else
       {
    ...
    else if($records_details['id'] !='' && $count > 2)
        {
         
       
             $aprroval_values = array('status'=>$rec['Status'],'approved_amount'=>$rec['ApprvdAmt'],'payment_status'=>$rec['PaymntSts'],'update_ts'=>date('Y-m-d H:i:s',strtotime('+4 hour +30 minutes')));
             $array_merge =  array_merge($records_details,$aprroval_values);         
            $this->db->where('id',$array_merge['id']);
          
           $this->db->update('schoolnew_maintenance',$array_merge);
            $affectedRows=$this->db->affected_rows();
        ...
        {
          $date_merge = array('creation_ts'=> date('Y-m-d H:i:s',strtotime('+4 hour +30 minutes')),'update_ts'=> date('Y-m-d H:i:s',strtotime('+4 hour +30 minutes')));
          $final_details = array_merge($date_merge,$records_details);
          $this->db->insert('schoolnew_maintenance',$final_details);
          $affectedRows=$this->db->affected_rows();
  ...
  public function Maintnce_InsrtUpdtStatus($rec)
  {

    $count=count($rec);
   
  
    $records_details = array('id'=>$rec['IndxID'],'maintenance_id'=>$rec['MaintnceId'],'attachment'=>$rec['Photo'],'remarks'=>$rec['Remrks'],
    'status'=>$rec['Stats']);
    ...
    if($count == 1)
    {
      $params = array('active'=>0);
      $this->db->where('id',$records_details['id']);
      $this->db->update('schoolnew_maintenance_updates',$params);
       $affectedRows=$this->db->affected_rows();
       ...
       else
       {
    ...
    else if($records_details['id'] !='' && $count > 2)
        {
             ...
             $aprroval_values = array('update_ts'=>date('Y-m-d H:i:s',strtotime('+4 hour +30 minutes')));
             $array_merge =  array_merge($records_details,$aprroval_values);         
            $this->db->where('id',$array_merge['id']);
          
           $this->db->update('schoolnew_maintenance_updates',$array_merge);
            $affectedRows=$this->db->affected_rows();
        ...
        else
        {
          ...
          $data_sent = array('maintenance_id'=>$rec['MaintnceId'],'attachment'=>$rec['Photo'],'remarks'=>$rec['Remrks'],
          'status'=>$rec['Stats'],'creation_ts'=> date('Y-m-d H:i:s',strtotime('+4 hour +30 minutes')),'update_ts'=> date('Y-m-d H:i:s',strtotime('+4 hour +30 minutes')));
       
          $this->db->insert('schoolnew_maintenance_updates',$data_sent);
          $affectedRows=$this->db->affected_rows();
  ...
  public function Billing_InsrtUpdt($rec)
  {
    $count=count($rec);
    $records_details = array('id'=>$rec['IndxID'],'school_id'=>$rec['SchlID'],'type'=>$rec['Typ'],'bill_amount'=>$rec['BillAmt'],'billing_month'=>$rec['BillMonth'],'billing_year'=>$rec['BillYear'],
    'attachment'=>$rec['BillPhoto'],'remarks'=>$rec['Remrks'],'status'=>$rec['Status'],'transaction_status'=>$rec['TrnsactnSts'],'active'=>$rec['DataSts']);
    
    if($count == 1)
    {
      $params = array('active'=>0);
      $this->db->where('id',$records_details['id']);
      $this->db->update('schoolnew_bill_charges',$params);
       $affectedRows=$this->db->affected_rows();
       ...
       else
       {
    ...
    else if($records_details['id'] !='' && $count > 2)
        {
          ...
          $array_merge =  array_merge($records_details,$aprroval_values);         
            ...
            $this->db->where('id',$array_merge['id']);
            $this->db->update('schoolnew_bill_charges',$array_merge);
            $affectedRows=$this->db->affected_rows();
            if($affectedRows == 1)
            {
              $affectedRows = 1;
        ...
        else
        {
          $date_merge = array('creation_ts'=> date('Y-m-d H:i:s',strtotime('+4 hour +30 minutes')),'update_ts'=> date('Y-m-d H:i:s',strtotime('+4 hour +30 minutes')));
          $final_details = array_merge($date_merge,$records_details);
          $this->db->insert('schoolnew_bill_charges',$final_details);
          $affectedRows=$this->db->affected_rows();
  ...
  public function Stipend_InsrtUpdt($rec)
  {
    

    $count=count($rec);
    $records_details = array('id'=>$rec['IndxID'],'student_id'=>$rec['StudID'],'student_bank_id'=>$rec['StuBankID'],'date_of_checked'=>$rec['DateOfCheckd'],
    'previous_balance'=>$rec['PrevBalnc'],'current_balance'=>$rec['CurntBalnc'],'remarks'=>$rec['Remrks']);
    
    if($count == 1)
    {
      $params = array('active'=>0);
      $this->db->where('id',$records_details['id']);
      $this->db->update('studentsnew_stipend',$params);
       $affectedRows=$this->db->affected_rows();
       ...
       else
       {
    ...
    else if($records_details['id'] !='' && $count > 2)
        {
          ...
          $array_merge =  array_merge($records_details,$aprroval_values);         
            ...
            $this->db->where('id',$array_merge['id']);
            $this->db->update('studentsnew_stipend',$array_merge);
            $affectedRows=$this->db->affected_rows();
            if($affectedRows == 1)
            {
              $affectedRows = 1;
        ...
        else
        {
          $date_merge = array('creation_ts'=> date('Y-m-d H:i:s',strtotime('+4 hour +30 minutes')),'update_ts'=> date('Y-m-d H:i:s',strtotime('+4 hour +30 minutes')));
          $final_details = array_merge($date_merge,$records_details);
          $this->db->insert('studentsnew_stipend',$final_details);
          $affectedRows=$this->db->affected_rows();
  ...
  public function MaintnceStatusGet($id,$maintance_id)
  {
    
    if(!empty($maintance_id))
    {
 
      $SQL="select id as IndxID, maintenance_id as MaintnceID,attachment as Attchmnt,remarks as Remrks,status as Stats from schoolnew_maintenance_updates where active=1 and maintenance_id=$maintance_id";
   
    }
    else if(!empty($id))
    {
   
      $SQL="select id as IndxID, maintenance_id as MaintnceID,attachment as Attchmnt,remarks as Remrks,status as Stats from schoolnew_maintenance_updates where active=1 and id=$id";
  ...
  public function MaintnceTypeGet()
  {
    $SQL="select id as MaintnceID,maintenance_type as MaintnceType from schoolnew_expense_head where category = 1 and isactive = 1;";
    $query=$this->db->query($SQL);
    return $query->result_array();
  }
  public function GetBranck_IFSC($ifsc)
  {
    $SQL="select id as BranchId, bank_id as BankId,bank_name as BankNam,branch as BranchNam,branch_add as BranchAddrs,contact_no as MobNo,city as City,ifsc_code as IFSC,micr_code as MICR  from schoolnew_branch where  ifsc_code='$ifsc'";
    $query=$this->db->query($SQL);
    return $query->result_array();
  }
  public function GetStipend_Det($std_id)
  {

    $SQL="SELECT id as IndxId,student_id as  StudID,student_bank_id as StuBankID,date_of_checked as DateOfCheckd, current_balance as CurntBalnc, previous_balance as PrevBalnc, remarks as Remrks   FROM studentsnew_stipend where student_id=$std_id and active=1 ORDER BY id DESC LIMIT 1;";
    $query=$this->db->query($SQL);
    return $query->result_array();  
  }
  public function GetStipend_Histry($std_id)
  {
    $SQL="SELECT id as IndxId,student_id as  StudID,student_bank_id as StuBankID,date_of_checked as DateOfCheckd, current_balance as CurntBalnc, previous_balance as PrevBalnc, remarks as Remrks   FROM studentsnew_stipend where student_id=$std_id and active=1";
    $query=$this->db->query($SQL);
    return $query->result_array(); 
  }
  public function GetImgUpload_Det($schl)
  {
    $SQL="SELECT id as IndxId,file_name as FileNam  FROM schoolnew_smc_resolution where school_id=$schl";
    $query=$this->db->query($SQL);
    return $query->result_array();
  }
  public function GetStdBnk_Det($rec)
  {
   ...
   $section=$rec['section'];
    ...
    $SQL="
    select a.id as IndxID,a.student_bank_id as StuBankID, 
        b.name as StdNam,b.class_studying_id  as Class,b.class_section as Section,b.school_id as SchlID, 
        a.current_balance as CurrBalance,a.student_id as StudID
    from studentsnew_stipend a
    inner join students_child_detail b on a.student_id=b.id
    where b.school_id=$school_id and b.class_studying_id=$class_id and b.class_section='$section' and a.id in 
    
    (
        select max(id) from studentsnew_stipend
        group by student_id
    )
    order by b.id;
    ";
  ...
  public function ImgUpld_InsrtUpdt($rec)
  {
    $records_details = array('id'=>$rec['IndxID'],'school_id'=>$rec['SchlID'],'file_name'=>$rec['FileNam']);
    $schl_id=$records_details['school_id'];  

    if(!empty($records_details['id']))
    {
      ...
      $array_merge =  array_merge($records_details,$aprroval_values);         
        ...
        $this->db->where('id',$array_merge['id']);
        $this->db->update('schoolnew_smc_resolution',$array_merge);
        $affectedRows=$this->db->affected_rows();
        if($affectedRows == 1)
        {
          $affectedRows = 1;
    ...
    else
    {
      ...
      $date_merge = array('creation_ts'=> date('Y-m-d H:i:s',strtotime('+4 hour +30 minutes')),'update_ts'=> date('Y-m-d H:i:s',strtotime('+4 hour +30 minutes')));
      $final_details = array_merge($date_merge,$records_details);
      $this->db->insert('schoolnew_smc_resolution',$final_details);
      $affectedRows=$this->db->affected_rows();
