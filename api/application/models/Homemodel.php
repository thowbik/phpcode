...
class Homemodel extends CI_Model
{
  ...
  public function getTeacherlist($school_id) {
	...
	public function school_needs_del_csr_material_master($data_id)
	{
	    

		$query = 'SELECT * FROM csr_school_requirements left join csr_contribution_details ON csr_school_requirements.id = csr_contribution_details.req_id  WHERE csr_contribution_details.status = "FUNDS_RECEIVED" and mat_id = ' . $data_id;
        $result = $this->db->query($query)->result();
		 ...
		 if(count($result) > 0)
		{
		    ...
		    $data = array('data' => "Already Exist Requirements!Unble to Delete");
			return $data;
			...
			$this->db->where('csr_school_requirements.mat_id',$data_id);
	...
	public function delete_csr_wise_update_details($update_id)
	{
	    

		$query = 'SELECT * FROM csr_school_req_updates WHERE csr_school_req_updates.id = ' . $update_id;
        $result = $this->db->query($query)->result();
		 ...
		 if($result)
		{
		    ...
		    $data = array('isactive' => '0');
			$this->db->where('csr_school_req_updates.id',$update_id);
			return $this->db->update('csr_school_req_updates', $data);
...
**************************************************************************************************************************/
    ...
    function getSchoolInfo($school_udise,$doclist){
        if($doclist==1){
			$SQL="select mgmt_app_status.school_id,mgmt_app_doc_uploads.doc_type,mgmt_app_doc_uploads.doc_id,mgmt_app_doc_uploads.user_type,mgmt_app_doc_uploads.doc_filepath,mgmt_app_doc_uploads.doc_name,mgmt_app_doc_uploads.doc_name_coded,mgmt_app_doc_uploads.doc_inspect_issue_authority,mgmt_app_doc_uploads.doc_inspect_date,mgmt_app_doc_uploads.doc_issue_date,mgmt_app_doc_uploads.doc_file_num,mgmt_app_doc_uploads.doc_file_date,mgmt_app_doc_uploads.doc_valid_from_date,mgmt_app_doc_uploads.doc_valid_upto_date,mgmt_app_doc_uploads.doc_issue_place,mgmt_app_doc_uploads.doc_survey_no,mgmt_app_doc_uploads.doc_details,mgmt_app_doc_uploads.ifs_code,mgmt_app_doc_uploads.challan_date,mgmt_app_doc_uploads.challan_no,mgmt_app_doc_uploads.fee_paid,mgmt_app_doc_uploads.isactive,mgmt_app_doc_uploads.created_at,mgmt_app_doc_uploads.updated_at from mgmt_app_doc_uploads join mgmt_app_status on mgmt_app_status.app_id=mgmt_app_doc_uploads.app_id and
mgmt_app_status.updated_at=(select max(updated_at) from mgmt_app_status where school_id=".$school_udise." and (order_valid_upto_date
<NOW() or order_valid_upto_date is null)) where app_type=1 and school_id=".$school_udise.";";
		}else{
        $SQL="SELECT *,schoolnew_basicinfo.udise_code as udise_code,schoolnew_basicinfo.block_id as bid,schoolnew_stdcode_mas.id as
sid FROM schoolnew_basicinfo
...
LEFT JOIN schoolnew_stdcode_mas ON schoolnew_stdcode_mas.id=schoolnew_basicinfo.office_std_code
...
left join mgmt_app_status on mgmt_app_status.school_id=schoolnew_basicinfo.school_id and app_type=1
WHERE
...
students_child_detail.email,students_child_detail.father_qualify, students_child_detail.mother_qualify, students_child_detail.guardian_qualify,
...
baseapp_bloodgroup.group,a.occu_name as father_occ,b.occu_name as mother_occ,baseapp_parentincome.income_value,schoolnew_mediumofinstruction.MEDINSTR_DESC,schoolnew_district.district_name,baseapp_religion.religion_name,baseapp_sub_castes.caste_name'); 
       ...
       $this->db->where('school_id',$school_id);
       $this->db->where('transfer_flag',0);
       $this->db->where('students_child_detail.name');
  ...
  function student_list($school_id,$classid,$subid,$termid,$sectionid)
   {  
    if($termid == '1'){
      $tblname = 'schoolnew_qstudent_markterm1';
    }elseif($termid == '2'){
      $tblname = 'schoolnew_qstudent_markterm2';
    }elseif($termid == '3'){
      $tblname = 'schoolnew_qstudent_markterm3';
    } 
    //echo $tblname;
    //die();
    ...
    $subj_id = $this->getsubjectflag($subid);
    
    $sql = "SELECT a.id as stuid, a.name, a.gender, a.class_studying_id, a.unique_id_no, a.class_section, 
    (select id from ".$tblname." where student_id = a.id and term_id = ".$termid." AND school_id = ".$school_id.") as updid, 
    b.term_id,b.book".$subj_id."_id as book_id, b.attend".$subj_id." as attendance, attreason".$subj_id." as attreason,
    b.student_id, b.student_name, 
    b.FAA".$subj_id." as FAA, b.FAB".$subj_id." as FAB,b.FAC".$subj_id." as FAC, b.FAD".$subj_id." as FAD, 
    b.FBA".$subj_id." as FBA, b.FBB".$subj_id." as FBB,b.FBC".$subj_id." as FBC, b.FBD".$subj_id." as FBD, 
    b.FAM".$subj_id."  as FAM, b.SAM".$subj_id." as SAM, b.status
    FROM students_child_detail a 
    LEFT JOIN ".$tblname." b ON a.id = b.student_id AND b.term_id = ".$termid." AND b.book".$subj_id."_id = ".$subid." 
    WHERE a.school_id = ".$school_id." AND a.class_studying_id = ".$classid.$concat_where." AND a.transfer_flag = 0 order by a.name" ;
    // return $sql; 
    $query = $this->db->query($sql);
    return $query->result_array();  
   }
   function getsubjectflag($subid){
    $subject_sql = "SELECT subject FROM schoolnew_textbooks_list where id = ".$subid.";";
    $subject_query = $this->db->query($subject_sql);
    $subject = $subject_query->result_array();
    $subject_id = $subject['0']['subject'];
    if($subject_id == 48 || $subject_id == 96 || $subject_id == 94 || $subject_id == 95 || $subject_id == 45){
      $subj_id = 1;
    }elseif($subject_id == 46){
      $subj_id = 2;
    }elseif($subject_id == 3){
      $subj_id = 3;
    }elseif($subject_id == 7 || $subject_id == 4){
      $subj_id = 4;
    }elseif($subject_id == 8){
      $subj_id = 5;
    }
   ...
   function update_student_attendance($studentabsent,$attendancevalue)
  {
    ...
    *************************************************************/
    ...
    function ordercopylist($module){
        ...
        schoolnew_school_department.department
...
UNION ALL SELECT MAX(temp_id) as school_id FROM newschool_details) AS regid) AS tempid_create;";
    ...
    function insertorupdatedata1($data){
     // return $this->db->insert('newschool_details', $data); 
     // print_r($data);//die();
      $data1 = array(
        'app_type'            => '1',    
        ...
        'sch_directorate_id'  => $data['sch_directorate_id'],
        'created_at'        => $data['created_at']
        );      
      $mgmt_app = $this->db->insert('mgmt_app_status', $data1);
      //$app_id = $data['app_id'];
      sleep(5);
      if($mgmt_app!='0'){
        $this->db->select('id'); 
        ...
        if(!empty($mgmt_app)){
            $data2 = array(
              ...
              'emis_user_id'        => $id,
              'emis_usertype'       => '22',
              'emis_username'       => $data['app_id'],
              'emis_password'       => $data['emis_password'],
              'ref'                 => $data['ref'],  
              'email'               => $data['email']
            // 'created_at'        => $created_date
            ); 
            return $result = $this->db->insert('emis_usernew', $data2);
        }
      }else{
        return false;
      }
    ...
    public function loginverfication($temp_id,$emis_password){
        $sql = "select * from emis_usernew where emis_username='".$temp_id."' and emis_password='".$emis_password."';";
        $query = $this->db->query($sql);
        return $query->result_array();    
    }
    ...
    function findtablename($columnname){
        $sql="SELECT DISTINCT TABLE_NAME FROM INFORMATION_SCHEMA.COLUMNS WHERE COLUMN_NAME IN ('".$columnname."')
                AND TABLE_SCHEMA='tnschools_working' and TABLE_NAME in('newschool_details','newschool_detailsland','schoolnew_natureofconst_entry' 
...
/******************************************pearlin**************************************/
 ...
 // $school_id = '47186';
  ...
  public function students_osc_reg($school_id) {  
      $exist_sql=' select class_section,class_studying_id,students_osc.school_id,students_osc.name,students_osc.unique_id_no,(select child_status from baseapp_osc where students_osc.present_status=baseapp_osc.id limit 1) as pre_stus ,training_type,ac_year  from students_osc
      left join students_child_detail on students_child_detail.unique_id_no=students_osc.unique_id_no;';     
      //$school_id = '5858';
      $sql='select class_section,class_studying_id,students_osc.school_id,students_osc.name,students_osc.unique_id_no,(select
      child_status from baseapp_osc where students_osc.present_status=baseapp_osc.id limit 1) as pre_stus
      ,training_type,ac_year from students_osc
      left join students_child_detail on students_child_detail.unique_id_no=students_osc.unique_id_no where students_osc.school_id='.$school_id.' and students_osc.ac_year !="" group by students_osc.unique_id_no,students_osc.ac_year;';
      $query = $this->db->query($sql);
      return $query->result();         
  }
...
return 1;
  ...
  {
    $this->db->select('student_scholor_nmms.id,
                       student_scholor_nmms.school_id,
                       student_scholor_nmms.student_id,
                       student_scholor_nmms.class,
                       student_scholor_nmms.section,
                       student_scholor_nmms.nmmsexam_passed,
                       student_scholor_nmms.remarks,
                       student_scholor_nmms.status,
                       student_scholor_nmms.ac_year,
                       students_child_detail.name,students_child_detail.gender,community_name');
    ...
    function getscheme_sports_all($school_id)
  {
    ...
    {
        $this->db->distinct();
        $this->db->select('a.id stu_id,a.name,a.unique_id_no as stu_unique_id,a.class_section,a.class_studying_id,b.*,a.school_id,c.school_name,phone_number,d.bank_name,d.micr_code,e.class_studying')
                 ->from('students_child_detail a')
                 ...
                 ->join('baseapp_class_studying e','e.id = a.class_studying_id','left')
    ...
    public function count_accountnumber($checkaccountnumber,$checkteachercode){
          $sql="select count(bank_acc) as countdata from students_special_cash_incentive where bank_acc=".$checkaccountnumber." ;";
          $query = $this->db->query($sql);
    ...
    ****/
    public function save_special_cash_details($cash_details,$school_id)
    {
      ...
      { 
        $cash_details['updated_by'] = $school_id;
...
baseapp_bloodgroup.group,a.occu_name as father_occ,b.occu_name as mother_occ,baseapp_parentincome.income_value,schoolnew_mediumofinstruction.MEDINSTR_DESC,schoolnew_district.district_name,baseapp_religion.religion_name,baseapp_sub_castes.caste_name,students_transfer_certificate_details.*'); 
  ...
  public function emis_school_notebooks_Distribution_details($school_id)
  {
    ...
    from students_child_detail join schoolnew_notebooks_dist on schoolnew_notebooks_dist.school_id = students_child_detail.school_id and schoolnew_notebooks_dist.student_id= students_child_detail.id
    left  join  schoolnew_mediumofinstruction on schoolnew_mediumofinstruction.MEDINSTR_ID = schoolnew_notebooks_dist.medium
    join schoolnew_schemedetail on schoolnew_schemedetail.id = schoolnew_notebooks_dist.scheme_category WHERE students_child_detail.school_id =".$school_id." order by unique_id_no";
    $query = $this->db->query($sql);
    return $query->result(); 
  }
  ...
  public function emis_school_uniforms_Distribution_details($school_id,$sets)
  {
    ...
    from schoolnew_schemeindent join students_child_detail on students_child_detail.id = schoolnew_schemeindent.student_id
    join schoolnew_freescheme on schoolnew_freescheme.id  = schoolnew_schemeindent.scheme_id
    join baseapp_community on baseapp_community.id = students_child_detail.community_id
    join schoolnew_schemedetail on schoolnew_schemedetail.scheme_id = schoolnew_schemeindent.scheme_id AND schoolnew_schemedetail.id=schoolnew_schemeindent.scheme_category
    join schoolnew_schemeapplicable on schoolnew_schemeapplicable.scheme_category = schoolnew_schemedetail.id and schoolnew_schemedetail.scheme_id = schoolnew_schemeapplicable.scheme_id
    join schoolnew_mediumofinstruction on schoolnew_mediumofinstruction.MEDINSTR_ID = students_child_detail.education_medium_id
    where schoolnew_schemeindent.scheme_id = 1 and isactive = 1 and students_child_detail.school_id  = ".$school_id."  ".$where." ";
    $query = $this->db->query($sql);
    //echo $sql;
    return $query->result(); 
  }
  ...
  public function emis_school_buspass_Distribution_details($school_id)
  {
    ...
    from students_child_detail join  schoolnew_busindent on  schoolnew_busindent.student_id = students_child_detail.id 
    join schoolnew_mediumofinstruction on schoolnew_mediumofinstruction.MEDINSTR_ID = students_child_detail.education_medium_id
    join baseapp_community on baseapp_community.id = students_child_detail.community_id
  ...
  public function emis_school_Laptops_Distribution_details($school_id,$where)
  {
    ...
    from students_child_detail left  join schoolnew_computerindent on schoolnew_computerindent.student_id =students_child_detail.id
...
public function stud_cwsn_report($school)
{
  ...
  from students_ied left join students_child_detail on students_child_detail.unique_id_no=students_ied.student_id
  ...
  $sql="select ".$d." schoolnew_schemeindent.scheme_id,schoolnew_schemeindent.scheme_category,
   ...
   public function ankleboot($scheme,$dist,$blk,$class)
   {
    ...
    //for By ID Base
     ...
     public function  school_needs_csr_material_master_byid($id)
    {
   ...
   $this->db->join('csr_category_master','csr_category_master.id = csr_material_master.cat_id','left');
    $this->db->join('csr_subcategory_master','csr_subcategory_master.id = csr_material_master.sub_cat_id','left');
     $this->db->where('csr_material_master.id',$id);
      
     $query = $this->db->get();

           return $query->result();   
    }
    public function cate_byid($id)
    {
     $sql ="select id, cat_name, isactive  from  csr_category_master where id='".$id."'";
      $query = $this->db->query($sql);
          return $query->result();    
    }
     public function sub_cate_byid($id)
    {
     $sql ="select id, sub_cat_name, cat_id, isactive from  csr_subcategory_master where id='".$id."'";
       $query = $this->db->query($sql);
           return $query->result();   
    }
    ...
    public function get_school_needs_csr($school)
    {
       ...
       $sql="select csr_material_master.id,DEscription, material_name, sub_cat_name,cat_name,csr_school_requirements.created_by,
				school_id, mat_id, qty, csr_school_requirements.cat_id, csr_school_requirements.sub_cat_id,DATE_FORMAT(csr_school_requirements.created_on,'%d-%m-%Y') as created_on from csr_school_requirements
 ...
 public function get_slas_class7_report($school_id)
  {
    //print_r($school_id);
        ...
        $sql="select unique_id,students_slas_class7.name,school_name,tamil,english,maths,science,social,sum(tamil+english+maths+science+social) as total_marks from students_slas_class7
...
GROUP BY unique_id;";
  ...
  /*$sql = " SELECT a.id as stuid, a.name, a.name_tamil, a.gender, a.dob, a.community_id, a.religion_id, a.mothertounge_id, 
            a.class_studying_id, a.district_id, a.unique_id_no, a.school_id, a.transfer_flag, a.class_section,b.acc_reason1,
            b.school1_id,b.lang".$s_id." as lang,b.eng".$s_id." as eng,b.maths".$s_id." as maths,b.science".$s_id." as science,b.pract".$s_id."
            as pract,b.social".$s_id." as social,b.total".$s_id." as total, b.status".$s_id." as status,b.access".$s_id." as access,b.acc_reason".$s_id."
            as acc_reason,b.id as updid FROM students_child_detail a LEFT JOIN ".$tname." b ON a.id = b.student_id
            WHERE a.school_id = ".$school_id." AND a.class_studying_id = ".$classid.$concat_where." AND a.transfer_flag = 0 order by a.name;" ; */

  $sql = " SELECT a.id as stuid, a.name, a.gender, a.class_studying_id, a.unique_id_no, a.class_section, b.acc_reason1,
            b.school1_id,b.lang".$s_id." as lang,b.eng".$s_id." as eng,b.maths".$s_id." as maths,b.science".$s_id." as science,b.pract".$s_id."
            as pract,b.social".$s_id." as social,b.total".$s_id." as total, b.status".$s_id." as status,b.access".$s_id." as access,b.acc_reason".$s_id."
            as acc_reason,b.id as updid FROM students_child_detail a LEFT JOIN ".$tname." b ON a.id = b.student_id
            WHERE a.school_id = ".$school_id." AND a.class_studying_id = ".$classid.$concat_where." AND a.transfer_flag = 0 order by a.name;" ;   
...
//print_r($tname);
  ...
  function getallteacherlist($school)
  {
  $sql="select teacher_id,teacher_name from udise_staffreg where school_key_id =$school and archive = 1 order by teacher_name";
 
      $query = $this->db->query($sql);
      return $query->result_array();

  }
   function getallclasssections($school_id,$dist_id,$edu_dist_id){ 
    ...
    if(!empty($dist_id) || !empty($edu_dist_id)){
      //  echo $dist_id;die();
        if(!empty($dist_id)){
          ...
          $where = "where sc.edu_dist_id = ".$edu_dist_id."";
        }
        $SQL = "SELECT sc.district_id,sc.district_name,sc.block_name,sc.school_name,snsg.class_id,snsg.section,sc.udise_code,if(snsg.school_type=1,'Govt/Aided','UnAided') as School_Type, smi.MEDINSTR_DESC,coalesce(case when snsg.class_id in (11,12) then bgc.group_name else 'NA' end,'NA') as Group_name,
        coalesce(case when snsg.class_id in (11,12) then bgc.group_code else 'NA' end,'NA') as Group_code,
        sum(snsg.students) as Students FROM schoolnew_section_group snsg
        LEFT JOIN schoolnew_mediumofinstruction smi  ON
        smi.MEDINSTR_ID=snsg.school_medium_id
        LEFT JOIN baseapp_group_code bgc ON bgc.id=snsg.group_id
        left join students_school_child_count sc on sc.school_id=snsg.school_key_id
        $where
        group by sc.school_id,snsg.class_id,snsg.section,snsg.school_medium_id,snsg.group_id;";

        $query = $this->db->query($SQL);   
        //print_r($this->db->last_query());   
        return $query->result();  
    }else{    
        $this->db->select('schoolnew_section_group.id,schoolnew_section_group.class_id,schoolnew_section_group.section,schoolnew_section_group.group_id,schoolnew_section_group.school_type,schoolnew_section_group.school_medium_id,schoolnew_mediumofinstruction.MEDINSTR_DESC,baseapp_group_code.group_name,baseapp_class_studying.class_studying,schoolnew_section_group.students,udise_staffreg.teacher_name,udise_staffreg.teacher_id') 
        ...
        ->join('baseapp_class_studying','baseapp_class_studying.id = schoolnew_section_group.class_id','left') 
        ->join('udise_staffreg','udise_staffreg.teacher_id = schoolnew_section_group.class_teacher_id','left') 
	  ...
	  function getclasstype($school_id,$dist_id,$edu_dist_id)
	  {
      if(!empty($dist_id) || !empty($edu_dist_id)){
        if(!empty($dist_id)){
          $SQL = "SELECT district_name,school_name,low_class,high_class
                  FROM `students_school_child_count`
                  WHERE `district_id` = '.$dist_id.' group by school_id;";
        }
        else{
          $SQL = "SELECT low_class,high_class
                  FROM `students_school_child_count`
                  WHERE `edu_dist_id` = '.$edu_dist_id.' group by school_id;";
        }
        $query = $this->db->query($SQL);   
        //print_r($this->db->last_query());die;  
        return $query->result();   
	  ...
	  function check_student($school_id,$classname,$sectionname){
		    $this->db->select('count(name) as totalstudentscount'); 
        $this->db->where('school_id',$school_id);
        $this->db->where('class_studying_id',$classname); 		
		    $this->db->where('class_section', $sectionname);
        $this->db->where('transfer_flag',0); 		
        $query =  $this->db->get('students_child_detail');
        return $query->first_row()->totalstudentscount;
       
    }

    public function check_timetable($school_id,$class_id,$section,$insertthisid){
      $this->db->select('count(master_classid) as total_master_keys '); 
      $this->db->where('school_id',$school_id);
      $this->db->where('class_id',$class_id); 		
      $this->db->where('section', $section);
      $query =  $this->db->get('schoolnew_timetable_term_classwise');
      $count = $query->first_row()->total_master_keys;
      if($count > 0){
        $this->db->set('master_classid',$insertthisid); 
        $this->db->where('school_id',$school_id);
        $this->db->where('class_id',$class_id); 		
        $this->db->where('section', $section);
        $this->db->update('schoolnew_timetable_term_classwise'); 
      }


    }

    function edit_students_child_detail($records,$where){
      $this->db->trans_start();
      $this->db->where($where);
	  ...
	  function delete_class_data($deletedid){
         ...
         if ($this->db->trans_status() === false) {
              ...
              $result = array( 'message' => 'Class Not Found!.',
                               'dataStatus' => false );
              }
         }
         else{
    ...
    function add_class_data($records){
      $this->db->insert('schoolnew_section_group', $records);      
      $insertId = $this->db->insert_id();
      if($insertId){
        $this->check_timetable($records['school_key_id'],$records['class_id'],$records['section'],$insertId);
      }
      // return  $insertId;
      return ($this->db->affected_rows() > 0) ? true : false;
    }

    function edit_class_data($records,$id){
      $this->db->trans_start();
      $this->db->where('id', $id);
      $this->db->update('schoolnew_section_group', $records);
      $affectedRows=$this->db->affected_rows();
      $this->db->trans_complete();            
      if ($this->db->trans_status() === false) {
        ...
        if($affectedRows > 0 ) {
          ...
          $result = array( 'message' => 'Haven`t Updated Anything!.',
                           'dataStatus' => true );
        }
   }
   else{
   ...
   return $result;
	...
	function getschool_medium($school_id,$dist_id,$edu_dist_id)
	{
      //$school_id = $this->session->userdata('emis_school_id');
      //print_r($school_id);
      if(!empty($dist_id) || !empty($edu_dist_id)){
        ...
        $SQL = "SELECT sc.district_name,sc.school_name,sc.school_id,`schoolnew_mediumofinstruction`.*
        FROM `schoolnew_mediumentry`
        LEFT JOIN `schoolnew_mediumofinstruction` ON
        `schoolnew_mediumofinstruction`.`MEDINSTR_ID`=`schoolnew_mediumentry`.`medium_instrut`
        left join students_school_child_count sc on sc.school_id=schoolnew_mediumentry.school_key_id
        $where group by schoolnew_mediumentry.school_key_id;";
        $query = $this->db->query($SQL);   
        //print_r($this->db->last_query());die;  
        return $query->result();   
      }else{ 
	...
	{
	  $this->db->select('*');
    $this->db->where('group_description',1);
    $query = $this->db->get('baseapp_group_code');
    return $query->result();
	}
	function getschool_cate($school_id,$dist_id,$edu_dist_id){
    if(!empty($dist_id) || !empty($edu_dist_id)){
        if(!empty($dist_id)){
          $where ="where sc.district_id= ".$dist_id."";
        }
        ...
        $SQL = "SELECT sc.school_name,manage_cate_id
        FROM `schoolnew_basicinfo`
        left join students_school_child_count sc on sc.school_id=schoolnew_basicinfo.school_id
        $where group by schoolnew_basicinfo.school_id;";
        $query = $this->db->query($SQL);   
        //print_r($this->db->last_query());die;  
        return $query->result();   
    }else{ 
  ...
  /**Scholastic Starts Here**/  

  function common_save_for_scholastic($tname,$data){
    // print_r($data);
    // echo '777'.$tname;
    $this->db->insert($tname, $data);
  ...
  function common_update_for_scholastic($UpdateID,$tname,$data){
      $this->db->where('id', $UpdateID);      
      $this->db->update($tname, $data); 
      $result = $this->db->affected_rows();
      return $result;	           
  ...
  function common_update_student_absent_data($stuabs,$data,$tname)
  {
      $this->db->where('student_id',$stuabs);
      //return $this->db->update('schoolnew_qstudent_markdetails', $data);
      return $this->db->update($tname, $data);  
  ...
  function common_update_student_absent($stuabs,$attendancevalue,$tname)
  {
      $this->db->set('student_attendance',$attendancevalue); 
      $this->db->where('student_id',$stuabs);
      //$this->db->update('schoolnew_qstudent_markdetails');
  ...
  function common_insert_student_attendance($studentabsent,$attendancevalue,$tname)
  {    
      $this->db->set('student_attendance',$attendancevalue); 
      $this->db->where_in('student_id',$studentabsent);
      //$this->db->update('schoolnew_qstudent_markdetails');
  ...
  /**Scholastic Ends Here**/

  /**Blocklist for Resend password in DC starts here**/
  function getallschoolblock($district_id){
    if($district_id !=''){
      $this->db->select('id as block_id,block_name,block_type,district_id')
         ->from('schoolnew_block')
  ...
  /**Blocklist for Resend password in DC ends here**/

  /**School list for Resend Password in DC starts here**/
  public function get_blockwise_school_name($block_id,$sch_name)
  {
    if($block_id !=''){
      //$this->db->SELECT('a.school_name,a.udise_code,b.ref as pswd')
      $this->db->SELECT('a.school_name,a.udise_code')
                ->FROM('students_school_child_count a')
                //->JOIN('emis_userlogin as b','b.emis_username = a.udise_code ')
                ->WHERE('a.block_id',$block_id)
                ->like('a.school_name',$sch_name);
      $result = $this->db->get()->result(); 
      return $result;             
    }          
    // $sch_names = [];
    // if($block_id !=''){
    //   $this->db->SELECT('a.school_name,a.udise_code,b.ref')
    //           ->FROM('students_school_child_count a')
    //           ->JOIN('emis_userlogin as b','b.emis_username = a.udise_code ')
    //           ->WHERE('a.block_id',$block_id)
    //           ->like('a.school_name',$sch_name);
    ...
    //       {
    //           $sch_names[] = array('value'=>$res->school_name.' - '.$res->udise_code); 
    //           //$sch_names['pwd'] = $res->ref;
    //       }
    ...
    // }else
    // {
    //       $sch_names[] = array('value'=>'No Records..');
    // }
    // $temp['suggestions'] = $sch_names;
    // //print_r($temp);
    // return $temp; 
  }
  /**School list for Resend Password in DC starts here**/

  /**School login password for Resend Password in DC starts here**/
  function getPswd($udise_code)
  {
      $sql = "select * from emis_userlogin where emis_username = ".$udise_code.";";
      $query = $this->db->query($sql);
      $result = $query->result_array();
      return $result['0']['ref'];
  }
  ...
  function get_task_dtls(){
    $this->db->select('emis_timesheet.id, emis_timesheet.resource_id,emis_mst_resource.resource_name, emis_timesheet.timesheet_date, emis_timesheet.module_id, emis_timesheet.task_type_id,emis_mst_tasktype.task_description,emis_timesheet.duration_hrs, emis_timesheet.remarks')
         ->from('emis_timesheet')
         ->join('emis_mst_resource','emis_mst_resource.resource_id=emis_timesheet.resource_id','left')
         ->join('emis_mst_tasktype','emis_mst_tasktype.task_id=emis_timesheet.task_type_id','left')
         ->where('emis_timesheet.isactive','1')
         ->order_by('emis_timesheet.resource_id','asc');                 
             $query = $this->db->get();  
            //  print_r($this->db->last_query());die();
      return $query->result_array();
  }

  public function dtls_insrt($record,$table,$where_condition)
    {
    ...
    public function task_dtls_upt($update,$table,$where_condition)
    {
      $this->db->where($where_condition);
    ...
    public function get_module_id($tname,$where){
      $this->db->select($tname.'.id');//get id 
      $this->db->from($tname); //table name 
      $this->db->where($where);
      $result = $this->db->get();
    ...
    public function resource_dtls_upt($update,$table,$where_condition){
      $this->db->where_in('id',$where_condition);
      $this->db->update($table,$update);
      return $this->db->affected_rows();
    }

    function resources_dtls($data,$module_id){
      for($i=0;$i<count($data);$i++){ 
        $this->db->select('emis_resource_allotment.id');
        $this->db->from('emis_resource_allotment');
        $this->db->where('emis_resource_allotment.isactive', 1);
        $this->db->where('emis_resource_allotment.module_id', $module_id);
        $this->db->where('emis_resource_allotment.resource_id',$data[$i]['resource_id']);
        $id = $this->db->get()->row()->id;                    
        if(!empty($id)){
          $this->db->where('emis_resource_allotment.id',$id);
          $this->db->update('emis_resource_allotment', $data[$i]);
        }
        else{
          $data[$i]['isactive'] = 1;
          $data[$i]['module_id'] = $module_id;
          $data[$i]['created_on'] = date("Y-m-d");
          $this->db->insert('emis_resource_allotment', $data[$i]);
        }
      }
    }

    public function resources_vaildate($tname,$where,$wr){
      $this->db->select($tname.'.id');//get id 
      $this->db->from($tname); //table name 
      $this->db->where($where);
      if($wr){
        $this->db->where_in('resource_id',$wr);
      }
      $query = $this->db->get();  
      $r = $query->result_array();
      if(!empty($r)){
        foreach($r as $inx=>$val){
          $r[$inx] = $val['id'];
        }return $r;
      }else return [];
    ...
    public function school_nocwithcbse(){

    }

    
    public function newschlapplist_get($token){
      
      $sql = "select mgmt_app_status.app_id,newschool_details.school_name,mgmt_app_status.app_type,mgmt_app_status.status,mgmt_app_status.status_user from newschool_details left join mgmt_app_status ON newschool_details.temp_id = mgmt_app_status.app_id where 
      mgmt_app_status.app_type = 2 and ";

      if($token['emis_usertype']=='6'){               //BEO Login
        //echo "BEO";
        $block_id = $token['block_id'];
        $schl_mgmt_id = array('22','24');
        $sql .= " newschool_details.block_id = '$block_id' and mgmt_app_status.sch_mgmt_id IN ('" . implode("','", $schl_mgmt_id) . "');";
      }else if($token['emis_usertype']=='10'){        //DEO Login
        //echo "DEO";
        $edu_dist_id = $token['edu_district_id'];
        $schl_mgmt_id = array('22','23','24');
        $sql .= " newschool_details.edu_dist_id = '$edu_dist_id' and mgmt_app_status.sch_mgmt_id IN ('" . implode("','", $schl_mgmt_id) . "');";
      }else if($token['emis_usertype']=='9'){         //CEO Login
        //echo "CEO";
        $dist_id = $token['district_id'];
        $schl_mgmt_id = array('23','24');
        $sql .= " newschool_details.district_id = '$dist_id' and mgmt_app_status.sch_mgmt_id IN ('" . implode("','", $schl_mgmt_id) . "');";
      }else if($token['emis_usertype']=='5'){         //Director Login
        //echo "Director";
        $dist_id = $token['dist_id'];
        $schl_mgmt_id = array('22','23','24');
        $sql .= " newschool_details.district_id = '$dist_id' and mgmt_app_status.sch_mgmt_id IN ('" . implode("','", $schl_mgmt_id) . "');";
      }
    ...
    function insertorupdateschlreg3($data){  
        $part = $data['part'];
        unset($data['part']);
        $temp_id = $data['temp_id'];

        $sql = "select id from newschool_trustentry where temp_id = $temp_id;";
        $query = $this->db->query($sql);
        $result_data = $query->last_row(); 
      
        // $this->db->select('id');
        // $this->db->from('newschool_trustentry');
        // $this->db->where('newschool_trustentry.temp_id', $data['temp_id']);
        //$id = $this->db->get()->row()->id;
        
          if(!empty($result_data)){
            $id = $result_data->id;
            $this->db->where('newschool_trustentry.id',$id);
            $update = $this->db->update('newschool_trustentry', $data);
          }else{
            $data['timestamp'] = date("Y-m-d");
            $insert = $this->db->insert('newschool_trustentry', $data);
          } 
          if(!empty($insert) || !empty($update)){
              if($part=='3'){
                $data1['part_3'] = '1';
              }            
              $this->db->where('newschool_registercomplete.temp_id',$data['temp_id']);
              $update = $this->db->update('newschool_registercomplete', $data1);
              return $update;
          } 
    }
    function docuplddata($data){  
      $app_id = $data['app_id'];
      $doc_type = $data['doc_type'];
      $doc_id = $data['doc_id'];

      $sql = "select id from mgmt_app_doc_uploads where app_id = $app_id and doc_type = $doc_type and doc_id = $doc_id;";
      $query = $this->db->query($sql);
      $result_data = $query->last_row(); 
        ...
        if(!empty($result_data)){
          ...
          $this->db->where('mgmt_app_doc_uploads.id',$id);
          $result = $this->db->update('mgmt_app_doc_uploads', $data);
        }else{
          //die();
          $data['created_at'] = date("Y-m-d");
          $result = $this->db->insert('mgmt_app_doc_uploads', $data);
        } 
        return $result;
  }
  
  /**********************************************************************************
	Renewal start here do not change anything below -----by Magesh
************************************************************************************/
	public function getstageslist($sch_directorate_id,$app_type,$stage){
		$SQL="select * from mgmt_app_stages where  sch_directorate_id = '".$sch_directorate_id."' and app_type='".$app_type."' and ".$stage.";";
        $query = $this->db->query($SQL);
        return $query->result_array();
	}
	
	public function getstagelistall($sch_directorate_id,$app_id){
		
		$SQL="select * from mgmt_app_stages 
		join mgmt_app_tracking on mgmt_app_tracking.user_type_id_from=mgmt_app_stages.user_type_id 
		join mgmt_app_doc_uploads on mgmt_app_doc_uploads.user_type=mgmt_app_stages.user_type_id and mgmt_app_doc_uploads.app_id=".$app_id."
		where  sch_directorate_id = '".$sch_directorate_id."' and mgmt_app_stages.app_type=1 and mgmt_app_tracking.app_id=".$app_id.";";
        $query = $this->db->query($SQL);
        return $query->result_array();
	}
	
	public function getappid($app_type,$listapp){
		$sql="select max(app_id) as app_id from mgmt_app_status where ".$listapp." app_type=".$app_type.";";
		//echo($sql);
		//die();
		$query = $this->db->query($sql);
        return $query->result_array();
	...
	public function getdocmaster(){
		$sql="select * from mgmt_app_doc_master;";
		$query = $this->db->query($sql);
		return $query->result_array();
	}
	public function getschoollist($school_id){
		$sql="select * from mgmt_app_status where ".$school_id.";";
		$query = $this->db->query($sql);
        return $query->result_array();
	}
  
	
	public function renewalsave($data,$tablename,$reference){
		if(in_array($tablename,array("mgmt_app_status"))){
			$sql="select * from mgmt_app_status where app_id=".$reference['app_id'].";";
			$query = $this->db->query($sql);
			$result = $query->num_rows();
			if($result==1){
				$this->db->update($tablename,$data,$reference);
				return $data;
			}else if($result==0){
				$this->db->insert($tablename,$data);
				return $data;
			...
			//print_r($data);die();
			return $this->db->insert($tablename,$data[0]);
		}elseif(in_array($tablename,array("mgmt_app_doc_uploads"))){
			$token=json_decode(base64_decode(explode(".",getallheaders()['Token'])[1]),true);
			/*if(!$this->db->update_batch($tablename,$data,'app_id')){
				return $this->db->insert_batch($tablename,$data);
			}*/
	...
	public function getrenewalinfo($where){
		$sql = "select
schoolnew_basicinfo.school_id as school_id, udise_code, school_name, school_name_tamil, sch_shortname,
schoolnew_basicinfo.district_id as district_id, block_id, urbanrural, schoolnew_basicinfo.edu_dist_id, manage_cate_id,
schoolnew_basicinfo.sch_management_id, schoolnew_basicinfo.sch_cate_id as sch_cate_id,
schoolnew_basicinfo.sch_directorate_id as sch_directorate_id,district_name,block_name,
schoolnew_academic_detail.renewal_valid,schoolnew_academic_detail.yr_recgn_first,schoolnew_academic_detail.yr_last_renwl,
schoolnew_academic_detail.yr_recogn_schl,schoolnew_academic_detail.certifi_no,
datediff(now(),app_submit_date) as pending_days,
mgmt_app_status.district_id as statusdistrict_id,
mgmt_app_status.school_id as
statusschool_id,
mgmt_app_status.sch_directorate_id as statusdirectorateid,
mgmt_app_status.sch_mgmt_id as
statusmgmtid,
mgmt_app_status.sch_cate_id as statuscatid,
mgmt_app_status.app_id as app_id,
mgmt_app_status.app_type,
prev_cond_status,app_submit_date,
status,status_user,status_stage,order_filepath,order_filename,order_filename_coded,
order_conditions_filepath,order_conditions_filename,order_conditions_filename_coded,
order_rc_num,order_date,order_valid_from_date,order_valid_upto_date,order_from_class,
order_to_class,order_recog_number

from schoolnew_basicinfo
JOIN schoolnew_manage_cate ON schoolnew_manage_cate.id=schoolnew_basicinfo.manage_cate_id
JOIN schoolnew_management ON schoolnew_management.id=schoolnew_basicinfo.sch_management_id
JOIN schoolnew_school_department ON schoolnew_school_department.id=schoolnew_basicinfo.sch_directorate_id
JOIN baseapp_category ON baseapp_category.id=schoolnew_basicinfo.sch_cate_id
...
left join mgmt_app_status on mgmt_app_status.school_id=schoolnew_basicinfo.school_id and mgmt_app_status.app_type=1
...
where ".$where.";";
	...
	public function docslist($app_id){
		$sql="select mgmt_app_doc_uploads.id,app_id,user_type,doc_type,doc_id,doc_filepath,
doc_name,doc_name_coded,doc_inspect_issue_authority,doc_inspect_date,doc_issue_date,
doc_file_num,doc_file_date,doc_valid_from_date,doc_valid_upto_date,doc_issue_place,
doc_survey_no,doc_details,ifs_code,challan_date,challan_no,fee_paid,mgmt_app_doc_uploads.isactive,created_at,updated_at,doc_description from mgmt_app_doc_uploads
join mgmt_app_doc_master on mgmt_app_doc_master.id=mgmt_app_doc_uploads.doc_type
 where app_id=".$app_id.";";
		$query = $this->db->query($sql);
		return $query->result_array();
	}
	
	public function docsremarks($app_id){
		$sql="select app_id,doc_type,doc_id,user_type_id,user_remarks,created_at,updated_at from mgmt_app_doc_remarks where app_id=".$app_id.";";
		$query = $this->db->query($sql);
		return $query->result_array();
	}
	
	public function renewalstatuslist($school_id){
		$sql="select school_id,mgmt_app_status.app_id,(case when user_action=0 then 'Submitted' when (user_action=2 or user_action=-3) then 'Query Raised' when user_action=3 then 'Forwarded'
when user_action=1 then 'Approved' else 'NA' end) as Action,user_type_id_from,user_type_id_to,(case when user_type_id_from=1 then 'School'  when user_type_id_from=6 then 'BEO' when user_type_id_from=10 then 'DEO' when user_type_id_from=9 then 'CEO' else 'NA' end) as byfrom,(case when user_type_id_to=1 then 'School'  when user_type_id_to=6 then 'BEO' when user_type_id_to=10 then 'DEO' when user_type_id_to=9 then 'CEO' else 'NA' end) as byto,remarks,action_time,status from mgmt_app_status

left join mgmt_app_tracking on mgmt_app_tracking.app_id=mgmt_app_status.app_id
where school_id=".$school_id." and mgmt_app_status.app_type=1;";
		$query=$this->db->query($sql);
		return $query->result_array();
	}
	
	public function renewalapprove($data,$tablename,$reference){
	...
	public function renewalpdfview($where){
		$sql="select students_school_child_count.udise_code,district_name,block_name,edu_dist_name,students_school_child_count.school_name,school_type,school_type_id,management,category,cate_type,schoolnew_basicinfo.sch_directorate_id as schooldirectorateid,schoolnew_basicinfo.sch_management_id,address,pincode,mgmt_app_status.school_id,mgmt_app_status.sch_directorate_id as sch_directorate_id,mgmt_app_status.sch_mgmt_id,mgmt_app_status.sch_cate_id,app_id,app_type,prev_cond_status,app_submit_date,status,status_user,status_stage,order_filepath,order_filename,order_filename_coded,order_conditions_filepath,order_conditions_filename,order_conditions_filename_coded,order_rc_num,order_date,order_valid_from_date,order_valid_upto_date,order_from_class,order_to_class,order_recog_number,mgmt_app_status.created_at,mgmt_app_status.updated_at from students_school_child_count 
		join mgmt_app_status on mgmt_app_status.school_id=students_school_child_count.school_id
		join schoolnew_basicinfo on schoolnew_basicinfo.school_id=students_school_child_count.school_id
		join user_category ON user_category.id=mgmt_app_status.status_user where app_type=1 ".$where.";";
		$query=$this->db->query($sql);
		$result = $query->result_array();
		return $result;
	}
	
	public function dasboardrenewalcount($where,$usertype=0){
		$sql="select 'Beo Pending' AS user_type,count(1) as 'total',6 as user from mgmt_app_status 
join schoolnew_basicinfo on schoolnew_basicinfo.school_id=mgmt_app_status.school_id where app_type=1 and status_user=6 and status!=1 and ".$where."
UNION ALL
select 'Deo Pending' AS user_type,count(1) as 'total',10 as user from mgmt_app_status 
join schoolnew_basicinfo on schoolnew_basicinfo.school_id=mgmt_app_status.school_id where app_type=1 and status_user=10 and status!=1 and ".$where."
UNION ALL
select 'Ceo Pending' AS user_type,count(1) as 'total',9 as user from mgmt_app_status
join schoolnew_basicinfo on schoolnew_basicinfo.school_id=mgmt_app_status.school_id where app_type=1 and status_user=9 and status!=1 and ".$where."
UNION ALL
select 'Total Application' as user_type,count(1) as 'total',99 as user from mgmt_app_status
join schoolnew_basicinfo on schoolnew_basicinfo.school_id=mgmt_app_status.school_id where app_type=1 AND IF(".$usertype."=6,schoolnew_basicinfo.sch_directorate_id IN (16,18,27,29,32,34),IF(".$usertype."=9,schoolnew_basicinfo.sch_directorate_id IN (15,17,19,20,21,22,23,24,26,28,31,33),schoolnew_basicinfo.sch_directorate_id IS NOT NULL)) and ".$where."
UNION ALL
select 'Approved Application' as user_type,count(1) as 'total',98 as user from mgmt_app_status
join schoolnew_basicinfo on schoolnew_basicinfo.school_id=mgmt_app_status.school_id where status=1 and app_type=1 AND IF(".$usertype."=6,schoolnew_basicinfo.sch_directorate_id IN (16,18,27,29,32,34),IF(".$usertype."=9,schoolnew_basicinfo.sch_directorate_id IN (15,17,19,20,21,22,23,24,26,28,31,33),schoolnew_basicinfo.sch_directorate_id IS NOT NULL)) and ".$where."
UNION ALL
select 'Rejected Application' as user_type,count(1) as 'total',97 as user from mgmt_app_status
join schoolnew_basicinfo on schoolnew_basicinfo.school_id=mgmt_app_status.school_id where status=2 and app_type=1 AND IF(".$usertype."=6,schoolnew_basicinfo.sch_directorate_id IN (16,18,27,29,32,34),IF(".$usertype."=9,schoolnew_basicinfo.sch_directorate_id IN (15,17,19,20,21,22,23,24,26,28,31,33),schoolnew_basicinfo.sch_directorate_id IS NOT NULL)) and ".$where.";"; 
		//echo $sql;die();
		$query = $this->db->query($sql);
		return $query->result_array();
	}
	
	public function renewallist($section,$where,$group){
		$sql="select *,datediff(now(),app_submit_date) as pendingdays,count(1) as total from students_school_child_count 
join mgmt_app_status on mgmt_app_status.school_id=students_school_child_count.school_id
join user_category ON user_category.id=mgmt_app_status.status_user where app_type=1".$where." group by ".$group.";";
		//echo $sql; die();
		$query=$this->db->query($sql);
		return $query->result_array();
	}
	...
	/**************************************************************************
	End Renwal by Magesh
	**************************************************************************/
	
	/**************************************************************************
		DCF FORM DETAILS - By Ramco Magesh
	***************************************************************************/
		public function schooldcflist($where){
			$sql="select school_id,udise_code,school_name as SchoolName,sch_shortname as SchoolShortName,
district_name,block_name,edn_dist_name,management,department,manage_name,category_name,
localbody_name,schoolnew_localbody_all.name as localbodyallname,schoolnew_habitation_all.name as habitationname,
assembly_name,parli_name,clus_name,
schoolnew_basicinfo.district_id as DistrictId,schoolnew_basicinfo.block_id as BlockId,urbanrural as RuralUrbanStatus,
(case when urbanrural=1 then 'Rural' when urbanrural=2 then 'Urban' else 'NA' end) as urbanruralstatus,
schoolnew_basicinfo.edu_dist_id as EducationaldstId,manage_cate_id as ManagementCategoryId,sch_management_id as SchoolManagementId,
sch_cate_id as SchoolCategoryId,sch_directorate_id as
SchoolDirectorateId,local_body_id as LocaBodyId,lb_vill_town_muni as LocalBodyId,lb_habitation_id as
VillageWard,cluster_id as ClusterId,panchayat_id,municipal_id,address as address,pincode as Pincode,sch_email as
SchoolEmail,latitude as Latitude,longitude as Longitude,assembly_id as AssemblyId,parliament_id as
ParlimentId,curr_stat,curstat_date,school_type as SchoolType,minority_sch as MiniorityStatus,minority_grp as
MniorityGroup,minority_yr as MiniorityValidUpto,rte as SchoolRTEStatus,low_class as LowestClass,high_class as
HigestClass,hill_frst as SchoolInHillArea,resid_schl as ResidentialSchoolStatus,typ_resid_schl as
ResidentialSchoolType,shftd_schl as ShiftSchoolStatus,cwsn_scl as SpecialCWSNSchoolStatus,yr_estd_schl as
SchEstablishmentyear,yr_recogn_schl as SchfirstRecogYear,yr_last_renwl as LastRenewalYear,certifi_no as
RenewalCertificateNumber,yr_recgn_first as PriRecogFirstYear,yr_rec_schl_elem as UppPriRecogFirstYear,yr_rec_schl_sec as
SceRecogFirstYear,yr_rec_schl_hsc as HrSecRecogFirstYear,upgrad_prito_uprpri as
PritoUppPriUpgradationYear,upgrad_uprprito_sec as UppPritoSecUpgradationYear,upgrad_secto_higsec as
SectoHrSecUpgradationYear,spl_edtor as SpecialEducatorStatus,anganwadi as Anganwadi,anganwadi_schl as
AnganwadiLocation,anganwadi_center as AnganwadiNumber,angan_wrks as AnganwadiWorkerCount,anganwadi_stu_b as
AnganwadiBoysCount,anganwadi_stu_g as AnganwadiGirlsCount
from schoolnew_basicinfo
left join schoolnew_academic_detail on schoolnew_academic_detail.school_key_id=schoolnew_basicinfo.school_id
left join schoolnew_training_detail on schoolnew_training_detail.school_key_id=schoolnew_basicinfo.school_id
left join schoolnew_district on schoolnew_district.id=schoolnew_basicinfo.district_id
left join schoolnew_block on schoolnew_block.id=schoolnew_basicinfo.block_id
left join schoolnew_edn_dist_mas on schoolnew_edn_dist_mas.id=schoolnew_basicinfo.edu_dist_id
left join schoolnew_management ON schoolnew_management.id=schoolnew_basicinfo.sch_management_id
left join schoolnew_manage_cate on schoolnew_manage_cate.id=schoolnew_basicinfo.sch_management_id
left join schoolnew_school_department on schoolnew_school_department.id=schoolnew_basicinfo.manage_cate_id
left join schoolnew_category on schoolnew_category.id=schoolnew_basicinfo.sch_cate_id
left join schoolnew_local_body on schoolnew_local_body.id=schoolnew_basicinfo.local_body_id
left join schoolnew_localbody_all on schoolnew_localbody_all.id=schoolnew_basicinfo.lb_vill_town_muni
left join schoolnew_habitation_all on schoolnew_habitation_all.id=schoolnew_basicinfo.lb_habitation_id
...
left join schoolnew_cluster_mas on schoolnew_cluster_mas.clus_code=schoolnew_basicinfo.cluster_id
			where".$where.";";
		...
		public function dcfformsave($data,$tablename,$reference){
			//print_r($data);die();
			if(in_array($tablename,array("schoolnew_basicinfo"))){
				return $this->db->update($tablename,$data,$reference);
			}else if(in_array($tablename,array("schoolnew_academic_detail","schoolnew_training_detail"))){
				if($this->db->get_where($tablename,$reference)->num_rows()>0){
					return $this->db->update($tablename,$data,$reference);
				}else{
					return $this->db->insert($tablename,$data);
				}
			}else if(in_array($tablename,array("schoolnew_basic_info_history"))){
				return $this->db->insert($tablename,$data);
			}
		}
	
	/**************************************************************************
		END HERE DCF FORM DETAILS - By Ramco Magesh
	***************************************************************************/

  
    public function get_newschool_details($app_id){
      $sql="select id as details_inx_id, temp_id, academic_year, udise_code, school_name, manage_cate_id, sch_management_id, sch_directorate_id, district_id, block_id, urbanrural, local_body_type_id, village_panchayat_id, vill_habitation_id, assembly_id, parliament_id, edu_dist_id, address, pincode, email, website, school_stdcode, school_landline, corr_hm_stdcode, corr_hm_landline, mobile, corr_hm_mobile, schooltype, schoolcat, low_class, high_class from newschool_details where temp_id=".$app_id.";";
      $query = $this->db->query($sql);
      return $query->result_array();
    }
    public function get_newschool_detailsland($app_id){
      $sql="select id as detailsland_inx_id, temp_id, totland_sq, totland_sqm, totland_acre, landregnumber, landregdate, natural_barrier, barrier_details, with_premises, schoolfaci, expan_sq, playgroundfacil, playground_sq, playground_acre, alterarrange, alter_address, dist_bg, landowner, land_amount, period_lease, lease_date, wall_type, peri_bound, land_bound, total_block, class_und_con, desk, ramp, handrails, staffquater, quater_room, gent_toil_staff, ladie_toil_staff, gent_uri_staff, ladie_uri_staff, tot_inuse_boys, tot_notuse_boys, reason_notuse_boys, tot_inuse_girls, tot_notuse_girls, reason_notuse_girls, cswn_inuse_boys, cswn_notuse_boys, cwsn_reasonnotuse_boys, cwsn_inuse_girls, cwsn_notuse_girls, cwsn_reasonnotuse_girls, urinals_inuse_boys, urinals_notuse_boys, urinal_reasonnotuse_boys, urinals_inuse_girls, urinals_notuse_girls, urinal_reasonnotuse_girls, toi_flush_boys, toi_flush_girls, urinal_faci_boys, urinal_faci_girls, sanitory, additional_toilet, additional_sqft, napkin, napkin_func, napkin_nonfunc, incen, func_choolas, nonfunc_choolas, func_auto, nonfunc_auto, tot_tap_avail, tot_tap_func, schooltank, waterpurifier, rainwater, drinkingwater, drink_water, other_drinkwater, wellclose from newschool_detailsland where temp_id=".$app_id.";";
      $query = $this->db->query($sql);
      return $query->result_array();
      // return '';
    }

    public function get_newschool_classroomlevel_entry($app_id){
      $sql="select id as classroomlevel_entry_inx_id, school_key_id, app_id, school_type, num_rooms from newschool_classroomlevel_entry where app_id=".$app_id.";";
      $query = $this->db->query($sql);
      return $query->result_array();
    }
                                    
    public function get_newschool_library_entry($app_id){                              
      $sql="select id as library_entry_inx_id, school_key_id, app_id, library_type, num_books, ncert_books from newschool_library_entry where app_id=".$app_id.";";
      $query = $this->db->query($sql);
      return $query->result_array();
    }

    public function get_newschool_trustentry($app_id){
      $sql="select id as trustentry_inx_id, temp_id, same_campus_yn, trustname, trustplace, pincode, trust_register_yn, challan_ifsc, challan_number, challan_amount, challan_date, bldg_value1, bldg_value2, student_strength1, student_strength2, endowment_ifsc, endowment_certi, endowment_dep_date, endowment_mature_date, endowment_amount, corpus_ifsc, corpus_amount, corpus_date, corpus_account from newschool_trustentry where temp_id=".$app_id.";";
      $query = $this->db->query($sql);
      return $query->result_array();
    }
    public function get_newschool_registercomplete($app_id){
      $sql="select id as reg_complete_inx_id, temp_id, part_1, part_2, part_3, final_flag from newschool_registercomplete where final_flag = 1 and temp_id=".$app_id.";";
      $query = $this->db->query($sql);
      return $query->result_array();
    }

    public function get_app_status($app_id){
      $sql="select mgmt_app_status.status_stage as currnt_stage,mgmt_app_stages.stage as final_stage,mgmt_app_stages.user_type_id as approving_authority from mgmt_app_status 
      left join mgmt_app_stages on mgmt_app_status.app_type = mgmt_app_stages.app_type 
      and mgmt_app_status.sch_mgmt_id = mgmt_app_stages.sch_mgmt_id 
      and mgmt_app_status.sch_directorate_id = mgmt_app_stages.sch_directorate_id 
      and mgmt_app_status.sch_cate_id = mgmt_app_stages.sch_cate_id 
    ...
    function getdetailsbyudise($udise){
      $SQL="SELECT 
      (SELECT district_name FROM schoolnew_district WHERE id=schoolnew_basicinfo.district_id) AS district_name,
        (SELECT edn_dist_name FROM schoolnew_edn_dist_mas WHERE id=schoolnew_basicinfo.edu_dist_id) AS edn_dist_name,
        (SELECT block_name FROM schoolnew_block WHERE id=schoolnew_basicinfo.block_id) as block_name,
        school_name,
        (SELECT category_name FROM schoolnew_category WHERE id=schoolnew_basicinfo.sch_cate_id) AS school_category,
        (SELECT manage_name FROM schoolnew_manage_cate WHERE id=schoolnew_basicinfo.manage_cate_id) AS mana_cate,
        (SELECT management FROM schoolnew_management WHERE id=schoolnew_basicinfo.sch_management_id) AS sch_management,
        (SELECT department FROM schoolnew_school_department WHERE id=schoolnew_basicinfo.sch_directorate_id) AS sch_directorate,
        (SELECT COUNT(1) FROM students_child_detail WHERE students_child_detail.school_id=schoolnew_basicinfo.school_id AND gender=1) AS boys,
        (SELECT COUNT(1) FROM students_child_detail WHERE students_child_detail.school_id=schoolnew_basicinfo.school_id AND (gender=0 OR gender=2 OR gender=3)) AS girls,
        (SELECT COUNT(1) FROM udise_staffreg WHERE udise_staffreg.school_key_id=schoolnew_basicinfo.school_id AND gender=1) AS maleteacher,
        (SELECT COUNT(1) FROM udise_staffreg WHERE udise_staffreg.school_key_id=schoolnew_basicinfo.school_id AND (gender=0 OR gender=2 OR gender=3)) AS femaleteacher,
        school_id,schoolnew_basicinfo.website,sch_email,address,pincode,curr_stat
    FROM schoolnew_basicinfo WHERE udise_code=".$udise;
      //echo($SQL);
      $query = $this->db->query($SQL);
      return $query->result_array();
    }
    function getlistschools($where){
      $SQL="SELECT 
      (SELECT district_name FROM schoolnew_district WHERE id=schoolnew_basicinfo.district_id) AS district_name,
        (SELECT edn_dist_name FROM schoolnew_edn_dist_mas WHERE id=schoolnew_basicinfo.edu_dist_id) AS edn_dist_name,
        (SELECT block_name FROM schoolnew_block WHERE id=schoolnew_basicinfo.block_id) as block_name,
        ...
        (SELECT COUNT(1) FROM udise_staffreg WHERE udise_staffreg.school_key_id=schoolnew_basicinfo.school_id AND gender=1) AS maleteacher,
        (SELECT COUNT(1) FROM udise_staffreg WHERE udise_staffreg.school_key_id=schoolnew_basicinfo.school_id AND (gender=0 OR gender=2 OR gender=3)) AS femaleteacher,
        (SELECT COUNT(1) FROM udise_staffreg WHERE udise_staffreg.school_key_id=schoolnew_basicinfo.school_id AND (gender=0 OR gender=2 OR gender=3)) AS totalstaff
    FROM schoolnew_basicinfo
    JOIN schoolnew_academic_detail ON schoolnew_academic_detail.school_key_id=schoolnew_basicinfo.school_id ".$where;
      //echo($SQL);die();
      $query = $this->db->query($SQL);
      return $query->result_array();
    }

    function getliststrength($where){
      $SQL="SELECT 
      district_name,edu_dist_name,block_name,udise_code,school_name,total_b,total_g,total,teach_tot,nonteach_tot,totstaff 
      FROM students_school_child_count".$where;
      $query = $this->db->query($SQL);
      return $query->result_array();    
    }
    function getptrstrength($where){
      $SQL="SELECT * FROM (SELECT district_name,edu_dist_name,block_name,udise_code,school_name,total_b,total_g,total,teach_tot,nonteach_tot,totstaff,(total/teach_tot) AS ptr 
      FROM students_school_child_count) AS derptr".$where;
      $query = $this->db->query($SQL);
      return $query->result_array();    
    }

    public function mgmtappstatusupdate($table,$app_id){  

      $sql="select mgmt_app_stages.stage,mgmt_app_stages.user_type_id from mgmt_app_status 
      left join mgmt_app_stages on mgmt_app_status.app_type = mgmt_app_stages.app_type 
      and mgmt_app_status.sch_mgmt_id = mgmt_app_stages.sch_mgmt_id 
      and mgmt_app_status.sch_directorate_id = mgmt_app_stages.sch_directorate_id 
      and mgmt_app_status.sch_cate_id = mgmt_app_stages.sch_cate_id 
      where mgmt_app_stages.stage = '1' and mgmt_app_status.app_id=".$app_id.";";      
      $query = $this->db->query($sql);
      $result = $query->last_row();
      $data['status'] = '0';
      $data['status_user'] = $result->user_type_id;
      $data['status_stage'] = $result->stage;

      $sql="select * from mgmt_app_status where app_id = $app_id";
      $query = $this->db->query($sql);
      $present = $query->result_array();

        if(!empty($present)){
          $this->db->where('mgmt_app_status.app_id', $app_id);
    ...
    function details_insert($table,$arr)
   {
      $this->db->trans_start();
   ...
   function details_update($table,$update_data,$where)
   {
      $this->db->trans_start();
      $this->db->where($where);
   ...
   function details_count($table,$where)
   {
   ...
   public function details_delete($tname,$idarr,$app_id){
		$SQL="DELETE FROM ".$tname." WHERE id IN (".implode(',',$idarr).") and app_id = ".$app_id.";";
    $query = $this->db->query($SQL);
    $affectedRows=$this->db->affected_rows();
    return $affectedRows;
    }

    public function diksha_login(){ 
      $token = Common::userToken();
      $records['emis_username']=$token['emis_username'];
      $emis_usertype=$token['emis_usertype'];
      $emis_username = $records['emis_username'];
        $this->db->select('emisuser_teacher.emis_username,emisuser_teacher.emis_user_id,emisuser_teacher.emis_usertype,emisuser_teacher.emis_usertype1,emisuser_teacher.emis_password,emisuser_teacher.status,udise_staffreg.teacher_name,udise_staffreg.udise_code,udise_offreg.district_id,udise_offreg.office_area,udise_offreg.tamil_name,udise_staffreg.teacher_type,udise_staffreg.school_key_id,udise_staffreg.teacher_id,teacher_type.type_teacher,teacher_subjects.subjects,emisuser_teacher.mod1')
                  ->from('emisuser_teacher')
                  ...
                  ->join('udise_offreg','udise_offreg.off_key_id = udise_staffreg.school_key_id and udise_staffreg.school_key_id >= 90000','left' ) 
                  
                  ->where('emis_username',$records['emis_username']);

        // $sql = "SELECT emisuser_teacher.emis_username,emisuser_teacher.emis_user_id,
        // emisuser_teacher.emis_usertype,emisuser_teacher.emis_usertype1,emisuser_teacher.emis_password,
        // emisuser_teacher.status,udise_staffreg.teacher_name,udise_staffreg.udise_code,udise_offreg.district_id,
        // udise_offreg.office_area,udise_offreg.tamil_name,udise_staffreg.teacher_type,
        // udise_staffreg.school_key_id,udise_staffreg.teacher_id,teacher_type.type_teacher,
        // teacher_subjects.subjects,emisuser_teacher.mod1
        // FROM udise_staffreg
        // LEFT JOIN emisuser_teacher ON udise_staffreg.u_id = emisuser_teacher.emis_user_id and teacher_id= $emis_username and
        // emis_usertype1 >= '90000'
        ...
        // LEFT JOIN udise_offreg ON udise_offreg.off_key_id = udise_staffreg.school_key_id and udise_staffreg.school_key_id >=
        // '90000'
        // WHERE emis_usertype = $emis_usertype and emis_usertype1 >= '90000' and emis_username = $emis_username;";
        //$query = $this->db->query($sql);
        //$result = $query->row();        
        $result = $this->db->get()->first_row();
       // print_r($this->db->last_query());die();
        ...
        { 
                if($result->emis_usertype == '14' || $result->emis_usertype == '8'){
                  if($result->school_key_id < '90000'){
                      ...
                      $sql = "select udise_code from students_school_child_count where school_id = '$schl_id';";
                      $return_value =  $this->db->query($sql);
                      $school = $return_value->result_array();
                      //print_r($school['0']['udise_code']);die();
                      $result->rsa_school_id = $school['0']['udise_code'];
                  }else if($result->school_key_id > '90000'){
      ...
      public function studentcontenttransf(){
        $SQL="SELECT id as IndexID,label as Label,content_name as ContntNam,content_name_coded as ContntNamCoded,class as Class,subject as Sub,unit as Unit,episode as Episode,medium as Medium,file_type as FileType,batch as Batch,isactive as Status,page as Page FROM students_content_transfer where isactive = '1' order by unit asc";
        $query = $this->db->query($SQL);
        $result = $query->result_array();
        return $result;
      }
      public function schoolnew_basicinfo($emis_username){
        $SQL="select schoolnew_basicinfo.district_id,schoolnew_district.district_code,schoolnew_district.district_name,schoolnew_basicinfo.sch_email as UserEmail,schoolnew_district.email as DCEmail from emis_userlogin 
        join schoolnew_basicinfo on schoolnew_basicinfo.udise_code = emis_userlogin.emis_username 
        join schoolnew_district on schoolnew_basicinfo.district_id = schoolnew_district.id
        where emis_userlogin.status = 'Active' and emis_userlogin.emis_username=".$emis_username.";";
        $query = $this->db->query($SQL);
        $result = $query->result_array();
        return $result;
      }
      public function emisuser_teacher($emis_username){
        $SQL="select schoolnew_basicinfo.district_id,schoolnew_district.district_code,schoolnew_district.district_name,emisuser_teacher.email as TeacherEmail,udise_staffreg.email_id as TeacherEmail2,schoolnew_district.email as DCEmail from emisuser_teacher 
        join udise_staffreg on udise_staffreg.teacher_id =  emisuser_teacher.emis_username 
        join schoolnew_basicinfo on schoolnew_basicinfo.udise_code = udise_staffreg.udise_code
        join schoolnew_district on schoolnew_basicinfo.district_id = schoolnew_district.id
        where emisuser_teacher.status = 'Active' and emisuser_teacher.emis_username =".$emis_username.";";
        $query = $this->db->query($SQL);
        $result = $query->result_array();
        return $result;
      }
      public function emisuser_teacher2($emis_username){
        $SQL="select schoolnew_basicinfo.district_id,schoolnew_district.district_code,schoolnew_district.district_name,udise_staffreg.teacher_name from emisuser_teacher 
        join udise_staffreg on udise_staffreg.teacher_id =  emisuser_teacher.emis_username 
        join schoolnew_basicinfo on schoolnew_basicinfo.udise_code = udise_staffreg.udise_code
        join schoolnew_district on schoolnew_basicinfo.district_id = schoolnew_district.id
        where emisuser_teacher.status = 'Active' and emisuser_teacher.emis_username =".$emis_username.";";
      ...
      public function updatelogin($data,$table)
      {
        $this->db->where('emis_username',$data['emis_username']);
      ...
      public function getData($usertype)
      {
          $this->db->select('id as IndexID,field_name as FldName,alias_name as AlisName,editor_type as EdtrType,approving_authority_type as AppAuthType,user_type as UsrType,visiblity as Visiblty,approval_flag as ApprvlFlg')
          ->from('db_field_approval')
      ...
      public function getmobileno2($id){
        $SQL="select teacher_id,mbl_nmbr from udise_staffreg where u_id =".$id.";";
        $query = $this->db->query($SQL);
        $result = $query->result();
        return $result;        
      }
      public function getmobileno1($id){
        $SQL="select school_id,office_mobile from schoolnew_basicinfo where schoolnew_basicinfo.school_id =".$id.";";
        $query = $this->db->query($SQL);
        $result = $query->result();
        return $result;        
      }
      public function updatemobileno2($data,$id){
       
          // echo($data);die();
 
          $this->db->set('office_mobile',$data); 
          $this->db->where_in('school_id',$id);
          $this->db->update('schoolnew_basicinfo');
            $result = $this->db->affected_rows();
            return $result;
          // echo $result; die();              
      }
      public function updatemobileno1($data,$id){
        
          //print_r($data);die();
          $this->db->set('mbl_nmbr',$data); 
          $this->db->where_in('teacher_id',$id);
          $this->db->update('udise_staffreg');
            $result = $this->db->affected_rows();
          return $result;     
      }
      public function getSurvey($schid){
        $SQL="select students_school_child_count.school_type_id as SchlTypID,catty_id as SchlCatyID,schoolnew_mst_surveys.survey_id as SrvyID, survey_name as SrvyNam, frequency as Frequency from students_school_child_count
        left join  schoolnew_mst_surveys on schoolnew_mst_surveys.school_catty_id = students_school_child_count.catty_id and schoolnew_mst_surveys.school_type_id = students_school_child_count.school_type_id
        where school_id =".$schid.";";
        $query = $this->db->query($SQL);
        $result = $query->result();
        return $result;        
      }
      public function getQuestset($surid){
        $SQL="select id as QuestID,question as Quest,question_type as QuestTyp from schoolnew_mst_survey_questions where isactive = 1 and survey_id =".$surid.";";
        $query = $this->db->query($SQL);
        $result = $query->result_array();
        return $result;
      }
      public function surveyresponse($resp){
        if($this->db->insert_batch('schoolnew_survey_response',$resp)){
          return true;
        }
        else{
          return false;
      ...
      public function surveyresponsestatus($resp){
        // return $this->db->insert('student_transferhistory', $data)
        if($this->db->insert('schoolnew_survey_response_status',$resp)){
          return true;
        }
        else{
          return false;
      ...
      public function SchoolsUnderBRTE($BRTEID){
        $SQL="select schoolnew_brte_map.zonal_id,schoolnew_zonal_subschools.subschool_id as school_id,school_name from schoolnew_zonal_subschools 
        left join schoolnew_brte_map on schoolnew_zonal_subschools.zonal_id = schoolnew_brte_map.zonal_id and schoolnew_brte_map.isactive = 1
        left join schoolnew_basicinfo on schoolnew_basicinfo.school_id = schoolnew_zonal_subschools.subschool_id
        where schoolnew_brte_map.brte_id = ".$BRTEID." and schoolnew_zonal_subschools.isactive = 1";
        $query = $this->db->query($SQL);
        $result = $query->result_array();
        return $result;
      }
      public function getSocialCategory(){
          $SQL="select id as IndxID,social_cat as SocialCat from teacher_social_category";
          $query = $this->db->query($SQL);
          return $query->result();
      }

      public function getDiffAbled(){
        $SQL="select id as IndxID,da_code as DACode,da_code as DACode,da_name as DAName,mhrd_id as MHRDID from baseapp_differently_abled";
        $query = $this->db->query($SQL);
        return $query->result();
    }
