...
class Student_model extends Custom_Model {
		...
		public function	getSubjectsdata(){
			$this->db->SELECT('subject_id,ob_subject');
		...
		public function	getSubjects($class,$medium){
			$sql="select subject_id,ob_subject from subject where subject_id in (select distinct(subject_id) from chapter where class in ('".$class."') and medium_id in  ($medium))";
		...
		public function getLearningOutcome($chapter){		
			...
			FROM chapter c,medium m,ob_subject s,sub_topic t,skills sk,learning_objective lo 
		...
		public function	getsub_new($class,$medium){
			$this->db->SELECT('chapter.subject_id,subject.ob_subject');
			$this->db->FROM('chapter');
