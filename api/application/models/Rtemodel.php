...
/**Check Whether the student alloted in this school or not api starts here by wesley**/
      function alloted_school($schl_id,$reg_no){
        $sql = "select * from student_rte_applied_schools where register_no = $reg_no and school_id = $schl_id and isactive = 1 and (allot_status = 1 OR allot_status = 2);";
        $query = $this->db->query($sql);
        $result = $query->result_array();
        return $result;
      }
