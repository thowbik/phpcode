...
class AWSBucket {
    ...
    public function uploadFilesToAWS($bucket,$key,$secret,$foldername,$files,$storageClass='STANDARD'){
       $filesArray=array();
       $bash=null;
       foreach($files as $bashidx => $file){
            ...
            if(!is_array($file['name'])){
                if($file['error']==0){
                    $fn=$file['name'];
                    $fpath=$this->s3BucketUpload($bucket,$key,$secret,$folder,$file,$storageClass='STANDARD');
            ...
            elseif(is_array($file['name'])){
                foreach($file['name'] as $fidx => $fname){
                    if($file['error'][$fidx]==0){
                        ...
                        $fpath=$this->s3BucketUpload($bucket,$key,$secret,$folder,$alfile);
                        $filesArray[$bash][$j]['fname']=$fn;
                        $filesArray[$bash][$j]['fpath']=$fpath;
                        $j++;
                    }
    ...
    public function s3BucketUpload($bucket,$key,$secret,$foldername,$file,$storageClass='STANDARD'){
        $bucketName = $bucket;
        $IAM_KEY = $key;
        $IAM_SECRET = $secret;
        // Connect to AWS
